...
SaikuOlapQueryHelper.prototype.getAxis = function(a) {
    if (a in this.model().queryModel.axes)
        return this.model().queryModel.axes[a];
    Saiku.log("Cannot find axis: " + a)
}
;
SaikuOlapQueryHelper.prototype.removeFilter = function(a, b) {
    if (b && 1 < a.filters.length) {
        for (var c = -1, d = 0, e = a.filters.length; d < e; d++)
            a.filters[d].flavour == b && (c = d);
    ...
    _.each(c, function(c) {
        _.each(c.hierarchies, function(c) {
            _.each(c.levels, function(d) {
                if (d.selection && d.selection.parameterName && d.selection.parameterName === a)
                    return b = {
                        hierarchy: c,
                        level: d
                    },
    ...
    _.each(c, function(c) {
        _.each(c.hierarchies, function(c) {
            _.each(c.levels, function(c) {
                if (c.selection && c.selection.parameterName && c.selection.parameterName === a)
                    return b = c.selection.members,
                    !1
            })
        })
    });
...
SaikuOlapQueryHelper.prototype.includeMeasure = function(a) {
    var b = this.model().queryModel.details.measures, c = b.length, d, e = !1;
    if (b && !_.isEmpty(b)) {
        for (d = 0; d < c; d++)
            if (b[d].name === a.name) {
                e = !0;
...
SaikuOlapQueryHelper.prototype.addCalculatedMeasure = function(a) {
    a && (this.removeCalculatedMeasure(a.name),
    this.model().queryModel.calculatedMeasures.push(a))
}
;
SaikuOlapQueryHelper.prototype.editCalculatedMeasure = function(a, b) {
    b && (this.removeCalculatedMeasure(a),
    this.removeCalculatedMember(a),
    this.model().queryModel.calculatedMeasures.push(b))
}
;
SaikuOlapQueryHelper.prototype.removeCalculatedMeasure = function(a) {
    ...
    (a = _.findWhere(b, {
        name: a
    })) && -1 < _.indexOf(b, a) && (b = _.without(b, a),
    this.model().queryModel.calculatedMeasures = b)
}
;
...
SaikuOlapQueryHelper.prototype.addCalculatedMember = function(a) {
    a && (this.removeCalculatedMember(a.name),
    this.model().queryModel.calculatedMembers.push(a))
}
;
SaikuOlapQueryHelper.prototype.editCalculatedMember = function(a, b) {
    b && (this.removeCalculatedMeasure(a),
    this.removeCalculatedMember(a),
    this.model().queryModel.calculatedMembers.push(b))
}
;
SaikuOlapQueryHelper.prototype.removeCalculatedMember = function(a) {
    ...
    this.model().queryModel.calculatedMembers = b)
}
;
...
SaikuOlapQueryHelper.prototype.nonEmpty = function(a) {
    a ? (this.model().queryModel.axes.ROWS.nonEmpty = !0,
    this.model().queryModel.axes.COLUMNS.nonEmpty = !0) : (this.model().queryModel.axes.ROWS.nonEmpty = !1,
    this.model().queryModel.axes.COLUMNS.nonEmpty = !1)
}
;
var DateFilterModel = Backbone.Model.extend({})
  ...
  , LevelChildMember = Backbone.Model.extend({
    initialize: function(a, b) {
        ...
        this.levelUniqueName = b.levelUniqueName,
        this.mname = b.mname,
        this.mcaption = b.mcaption)
    },
    url: function() {
...
SaikuTableRenderer.prototype.render = function(a, b) {
    ...
    if ("undefined" != typeof this._data && !(null != this._data && null != this._data.error || null == this._data || this._data.cellset && 0 === this._data.cellset.length))
        if (this._options.htmlObject)
            c._options.hasOwnProperty("batch") && $(c._options.htmlObject).parent().parent().unbind("scroll"),
            _.defer(function(a) {
                c._options.hasOwnProperty("batch") && !c._options.hasOwnProperty("batchSize") && (c._options.batchSize = 1E3);
                    ...
                    if (c._options.hasOwnProperty("batch") && c._options.hasBatchResult) {
                        ...
                        var k = c._options.batchResult.length
                          , l = _.debounce(function() {
                            if (!d && 0 < k && b < k) {
                                d = !0;
                                for (var a = "", e = b, l = 0; b < k && l < h; l++,
                                b++)
                                    a += c._options.batchResult[b];
            ...
            return this.internalRender(this._data, c._options)
}
;
...
function genTotalDataCells(a, b, c, d, e) {
    ...
    for (var g = c.length - 1; 0 <= g; g--)
        if (a == c[g]) {
            for (var h = e[g][d[g]], k = 0; k < h.cells.length; k++)
                f += '\x3ctd class\x3d"data total"\x3e' + h.cells[k][b].value + "\x3c/td\x3e";
            d[g]++;
    ...
    for (var g = "", h = b; 0 <= h; h--)
        if (a == c[h]) {
            ...
            for (var m = 0; m < k.cells.length; m++) {
                var n = "\x26nbsp;";
                b == e.length - 1 && (k.captions && (n = e[h][d[h]].captions[m]),
                0 == h && 0 == d[h] && (n = k.captions ? n + "\x26nbsp;" : "",
                n += f ? "\x3cspan class\x3d'i18n'\x3eGrand Total\x3c/span\x3e" : "Grand Total"));
...
function totalIntersectionCells(a, b, c, d, e) {
    for (var f = ""; 0 <= b; b--)
        if (a == c[b]) {
            for (var g = e[b][d[b]], h = 0; h < g.cells.length; h++)
                f += '\x3ctd class\x3d"data total"\x3e\x26nbsp;\x3c/td\x3e';
            d[b]++;
        ...
        if (a == g[m]) {
            for (var n = 0; n < f[m][h[m]].cells.length; n++) {
                ...
                for (b = 0; b <= k; b++) {
                    ...
                    b == k && (f[m][h[m]].captions && (r = f[m][h[m]].captions[n]),
                    0 == m && 0 == h[m] && (r = f[m][h[m]].captions ? r + "\x26nbsp;" : "",
                    r += e ? "\x3cspan class\x3d'i18n'\x3eGrand Total\x3c/span\x3e" : "Grand Total"));
                    l += '\x3cth class\x3d"' + c + '"\x3e' + (e ? "\x3cdiv\x3e" + r + "\x3c/div\x3e" : r) + "\x3c/th\x3e"
                }
                c = {};
                b = {};
                for (r = 0; r < d[ROWS].length; r++)
                    c[r] = 0,
                    b[r] = d[ROWS][r][c[r]].width;
                for (r = 0; r < f[m][h[m]].cells[n].length; r++)
                    l += '\x3ctd class\x3d"data total"\x3e' + f[m][h[m]].cells[n][r].value + "\x3c/td\x3e",
                    l += totalIntersectionCells(r + 1, d[ROWS].length - 1, b, c, d[ROWS]);
                l += "\x3c/tr\x3e"
            }
            h[m]++;
...
function nextParentsDiffer(a, b, c) {
    for (; 0 < b--; )
        if (a[b][c].properties.uniquename != a[b][c + 1].properties.uniquename)
            return !0;
    return !1
}
function topParentsDiffer(a, b, c) {
    for (; 0 < c--; )
        if (a[b][c].properties.uniquename != a[b - 1][c].properties.uniquename)
            return !0;
    return !1
}
SaikuTableRenderer.prototype.internalRender = function(a, b) {
    var c = "", d = "", e = a.cellset, f = e ? e : [], g, h, k, l, m, n = 0, r = [], q = null , s = !1, t = !1, x = !1, z = [], y = !0;
    b && (q = b.hasOwnProperty("batchSize") ? b.batchSize : null ,
    y = b.hasOwnProperty("wrapContent") ? b.wrapContent : !0);
    var v = {};
    v[COLUMNS] = a.rowTotalsLists;
    v[ROWS] = a.colTotalsLists;
    for (var B = {}, C = {}, t = [ROWS, COLUMNS], d = 0; d < t.length; d++)
        B[t[d]] = [],
        C[t[d]] = [];
    if (v[COLUMNS])
        for (d = 0; d < v[COLUMNS].length; d++)
            C[COLUMNS][d] = 0,
            B[COLUMNS][d] = v[COLUMNS][d][C[COLUMNS][d]].width;
    for (var u = 0, G = f.length; u < G; u++) {
        var H = u - a.topOffset;
        g = 1;
        var I = t = l = h = !1;
        if (v[ROWS])
            for (d = 0; d < v[ROWS].length; d++)
                C[ROWS][d] = 0,
                B[ROWS][d] = v[ROWS][d][C[ROWS][d]].width;
        d = "\x3ctr\x3e";
        0 === u && (d = "\x3cthead\x3e" + d);
        for (var w = 0, J = f[u].length; w < J; w++) {
            var F = w - a.leftOffset
              , p = e[u][w];
            "COLUMN_HEADER" === p.type && (t = !0);
            if ("COLUMN_HEADER" === p.type && "null" === p.value && (null == k || w < k))
                d += '\x3cth class\x3d"all_null"\x3e\x26nbsp;\x3c/th\x3e';
            else if ("COLUMN_HEADER" === p.type) {
                null == k && (k = w);
                f[u].length == w + 1 ? l = !0 : m = e[u][w + 1];
                if (l)
                    "null" == p.value ? d += '\x3cth class\x3d"col_null"\x3e\x26nbsp;\x3c/th\x3e' : (v[ROWS] && (g = v[ROWS][u + 1][C[ROWS][u + 1]].span),
                    d += '\x3cth class\x3d"col" style\x3d"text-align: center;" colspan\x3d"' + g + '" title\x3d"' + p.value + '"\x3e' + (y ? '\x3cdiv rel\x3d"' + u + ":" + w + '"\x3e' + p.value + "\x3c/div\x3e" : p.value) + "\x3c/th\x3e");
                else {
                    var F = 1 < w && 1 < u && !h && w > k ? e[u - 1][w + 1].value != e[u - 1][w].value || e[u - 1][w + 1].properties.uniquename != e[u - 1][w].properties.uniquename : !1
                      , D = 999 < g ? !0 : !1;
                    p.value != m.value || nextParentsDiffer(e, u, w) || h || F || D ? ("null" == p.value ? d += '\x3cth class\x3d"col_null" colspan\x3d"' + g + '"\x3e\x26nbsp;\x3c/th\x3e' : (v[ROWS] && (g = v[ROWS][u + 1][C[ROWS][u + 1]].span),
                    d += '\x3cth class\x3d"col" style\x3d"text-align: center;" colspan\x3d"' + (0 == g ? 1 : g) + '" title\x3d"' + p.value + '"\x3e' + (y ? '\x3cdiv rel\x3d"' + u + ":" + w + '"\x3e' + p.value + "\x3c/div\x3e" : p.value) + "\x3c/th\x3e"),
                    g = 1) : g++
                }
                v[ROWS] && (d += genTotalHeaderCells(w - a.leftOffset + 1, u + 1, B[ROWS], C[ROWS], v[ROWS], y))
            } else if ("ROW_HEADER" === p.type && "null" === p.value)
                d += '\x3cth class\x3d"row_null"\x3e\x26nbsp;\x3c/th\x3e';
            else if ("ROW_HEADER" === p.type) {
                n == w ? h = !0 : m = e[u][w + 1];
                F = e[u - 1];
                D = !I && !h && (0 == w || !topParentsDiffer(e, u, w)) && p.value === F[w].value;
                I = !D;
                F = D ? "\x3cdiv\x3e\x26nbsp;\x3c/div\x3e" : '\x3cdiv rel\x3d"' + u + ":" + w + '"\x3e' + p.value + "\x3c/div\x3e";
                y || (F = D ? "\x26nbsp;" : p.value);
                var D = D ? "row_null" : "row"
                  , E = 0;
                if (!h && ("undefined" == typeof m || "null" === m.value)) {
                    var E = 1
                      , A = p.properties.dimension
                      , p = p.properties.level
                      , p = A in r ? r[A].length - r[A].indexOf(p) : 1
                      , A = w + 1;
                    for (; E < p && A <= n + 1 && "null" !== e[u][A]; A++)
                        E = A - w;
                    w = w + E - 1
                }
                d += '\x3cth class\x3d"' + D + '" ' + (0 < E ? ' colspan\x3d"' + E + '"' : "") + "\x3e" + F + "\x3c/th\x3e"
            } else if ("ROW_HEADER_HEADER" === p.type)
                d += '\x3cth class\x3d"row_header"\x3e' + (y ? "\x3cdiv\x3e" + p.value + "\x3c/div\x3e" : p.value) + "\x3c/th\x3e",
                h = !0,
                n = w,
                p.properties.hasOwnProperty("dimension") && (A = p.properties.dimension,
                A in r || (r[A] = []),
                r[A].push(p.properties.level));
                ...
                p.properties.hasOwnProperty("style") && (D = " style\x3d'background-color: " + p.properties.style + "' ");
                p.properties.hasOwnProperty("link") && (A = "\x3ca target\x3d'__blank' href\x3d'" + p.properties.link + "'\x3e" + A + "\x3c/a\x3e");
                p.properties.hasOwnProperty("arrow") && (E = "\x3cimg height\x3d'10' width\x3d'10' style\x3d'padding-left: 5px' src\x3d'./images/arrow-" + p.properties.arrow + ".gif' border\x3d'0'\x3e");
                d += '\x3ctd class\x3d"data" ' + D + "\x3e" + (y ? '\x3cdiv class\x3d"datadiv" alt\x3d"' + p.properties.raw + '" rel\x3d"' + p.properties.position + '"\x3e' : "") + A + E + (y ? "\x3c/div\x3e" : "") + "\x3c/td\x3e";
                v[ROWS] && (d += genTotalDataCells(F + 1, H, B[ROWS], C[ROWS], v, y))
            }
        }
        d += "\x3c/tr\x3e";
        g = "";
        v[COLUMNS] && 0 <= H && (g += genTotalHeaderRowCells(H + 1, B, C, v, y));
        s && q ? u <= q ? (t || x || (c += "\x3c/thead\x3e\x3ctbody\x3e",
        x = !0),
        c += d,
        0 < g.length && (c += g)) : (z.push(d),
        0 < g.length && z.push(g)) : (t || x || (c += "\x3c/thead\x3e\x3ctbody\x3e",
        x = !0),
        c += d,
        0 < g.length && (c += g))
    }
    b && (b.batchResult = z,
    b.hasBatchResult = 0 < z.length);
...
var SaikuChartRenderer = function(a, b) {
    ...
    if (!b && !b.hasOwnProperty("htmlObject"))
    ...
    this.rawdata && ("sunburst" == this.type ? this.process_data_tree({
        data: this.rawdata
...
SaikuChartRenderer.prototype.zoomout = function() {
    ...
    null === c || 0 === c.length ? $(this.el).find(".zoomout").hide() : 1 == c.length ? ($(this.el).find(".zoomout").hide(),
    a.keptVisibleDatumSet = [],
    pvc.data.Data.setVisible(b.datums(null , {
        visible: !1
    }), !0)) : 1 < c.length && (a.keptVisibleDatumSet.splice(c.length - 1, 1),
    b = b.datums(null , {
        visible: !1
    }).array(),
    _.intersection(a.keptVisibleDatumSet[c.length - 1], b).forEach(function(a) {
        a.setVisible(!0)
    }));
    a.render(!0, !0, !1)
}
...
SaikuChartRenderer.prototype.switch_chart = function(a, b) {
    if (null != b || void 0 != b) {
        if (null != b.chartDefinition || void 0 != b.chartDefinition)
            this.chartDefinition = b.chartDefinition;
        if (null != b.workspace || void 0 != b.workspace)
    ...
    null !== a && "" !== a && ("sunburst" == a ? ($(this.el).find(".zoombuttons a").hide(),
    this.type = a,
    c = c[a],
    this.sunburst(c),
    this.hasProcessed && this.render()) : c.hasOwnProperty(a) ? ($(this.el).find(".zoombuttons a").hide(),
    this.type = a,
    c = c[a],
    this.cccOptions = this.getQuickOptions(c),
    this.define_chart(),
...
SaikuChartRenderer.prototype.define_chart = function(a) {
    ...
    0 < f && (c.width = f);
    0 < g && (c.height = g);
    e && (c.hasOwnProperty("extensionPoints") && c.extensionPoints.hasOwnProperty("line_interpolate") && delete c.extensionPoints.line_interpolate,
    c.hasOwnProperty("extensionPoints") && c.extensionPoints.hasOwnProperty("area_interpolate") && delete c.extensionPoints.area_interpolate);
    a = {
        legend: {
            scenes: {
                item: {
        ...
        userSelectionAction: function(a) {
            ...
            if (1500 < d.datums().count())
                pvc.data.Data.setSelected(a, !0);
...
SaikuChartRenderer.prototype.render_chart_element = function(a) {
    ...
    if (!c || $(a.el).find(".canvas_wrapper").is(":visible"))
        $(a.el).find(".canvas_wrapper"),
        $(a.el).find(".canvas_wrapper").hide();
    try {
        c && $(a.el).find(".canvas_wrapper").show(),
        a.chart.render(),
        a.hasRendered = !0
    } catch (d) {
        $("#canvas_" + a.id).text("Could not render chart" + d)
    }
    if (a.chart.options && a.chart.options.animate)
        return !1;
...
SaikuChartRenderer.prototype.process_data_tree = function(a, b, c) {
    ...
    if ("undefined" != typeof a && "undefined" != typeof a.data && !(null !== a.data && null !== a.data.error || null === a.data || a.data.cellset && 0 === a.data.cellset.length)) {
        ...
        if (f && 0 < f.length) {
            var g = 0, h = 0, k = !1, l, m, n, r = function(a, b) {
                return a + b
            }
            ;
            l = 0;
            for (m = f.length; 0 === h && l < m; l++)
                for (var q = 0, s = f[l].length; q < s; q++) {
                    if (!k)
                        for (; "COLUMN_HEADER" == f[l][q].type && "null" == f[l][q].value; )
                            l++;
                    k = !0;
                    if ("ROW_HEADER_HEADER" == f[l][q].type) {
                        for (; "ROW_HEADER_HEADER" == f[l][q].type; )
                            b && d.metadata.push({
                                colIndex: q,
                                colType: "String",
                                colName: f[l][q].value
                    ...
                    if ("COLUMN_HEADER" == f[l][q].type) {
                        h = 0;
                        for (n = []; h <= l; )
                            "null" !== f[h][q].value && n.push(f[h][q].value),
                            h++;
                        b && d.metadata.push({
                            colIndex: q,
                            colType: "Numeric",
                ...
                if ("" !== f[l][0].value) {
                    ...
                    for (n = 0; n <= g; n++) {
                        if (f[l] && "null" === f[l][n].value) {
                            for (var e = d, t = 0; t < g && "null" === f[l][t].value; t++)
                                e = e[k[t]];
                            t > n && (n = t)
                        }
                        if (f[l] && "null" !== f[l][n].value) {
                    ...
                    n = _.clone(k);
                    e = g + 1;
                    for (t = f[l].length; e < t; e++) {
                        var x = f[l][e]
                          , z = x.value || 0
                          , y = 0 !== z
                          , v = x.properties.raw;
                        v && "null" !== v ? z = parseFloat(v) : "number" !== typeof x.value && parseFloat(x.value.replace(/[^a-zA-Z 0-9.]+/g, "")) && (z = parseFloat(x.value.replace(/[^a-zA-Z 0-9.]+/g, "")),
                        y = !1);
                        0 < z && y && (z = x.value && 0 <= x.value.indexOf("%") ? 100 * z : z);
                        s.push(z);
                        n.push({
                            f: x.value,
                            v: z
...
var DataSources = Backbone.Model.extend({
  ...
  , DimensionList = Backbone.View.extend({
    ...
    select: function(a) {
        ...
        a.hasClass("root") && (a.find("a").toggleClass("folder_collapsed").toggleClass("folder_expand"),
        a.toggleClass("collapsed").toggleClass("expand"),
        a.parents("li").find("ul").children("li").toggle(),
        a.hasClass("expand") && Saiku.events.trigger("workspace:expandDimension", this, null ));
        return !1
    },
        ...
        if ("QUERYMODEL" == this.workspace.query.model.type)
            if ($(a.target).parent().hasClass("ui-state-disabled"))
                ...
                var d = $(a.target).attr("level")
                  ...
                  , e = 0 < $(this.workspace.el).find(".workspace_fields ul.hierarchy[hierarchy\x3d'" + c + "']").length ? $(this.workspace.el).find(".workspace_fields ul[hierarchy\x3d'" + c + "'] a[level\x3d'" + d + "']").parent().show().parents(".fields_list_body").hasClass("rows") ? "ROWS" : "COLUMNS" : (0 < $(this.workspace.el).find(".workspace_fields .fields_list[title\x3d'ROWS'] ul.hierarchy").length ? $(this.workspace.el).find(".workspace_fields .fields_list[title\x3d'COLUMNS'] ul.connectable") : $(this.workspace.el).find(".workspace_fields .fields_list[title\x3d'ROWS'] ul.connectable")).parents(".fields_list").attr("title");
                f ? (f = $(a.target).attr("uniquename"),
                this.workspace.toolbar.$el.find(".group_parents").removeClass("on"),
                this.workspace.toolbar.group_parents(),
                this.workspace.query.helper.includeLevelCalculatedMember(e, c, d, f)) : this.workspace.query.helper.includeLevel(e, c, d);
  ...
  , Toolbar = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        _.bindAll(this, "call");
        // CR 10/2016 - removed Saiku logo from being displayed
  ...
  , Upgrade = Backbone.View.extend({
    ...
    render: function() {
        ...
        if (Settings.BIPLUGIN5) {
            ...
            if (void 0 != Settings.LICENSE.licenseType && "trial" != Settings.LICENSE.licenseType && "Open Source License" != Settings.LICENSE.licenseType)
                return this;
            ...
            if (void 0 != Settings.LICENSE.licenseType && "trial" != Settings.LICENSE.licenseType && "Open Source License" != Settings.LICENSE.licenseType)
                return this;
            "trial" === Settings.LICENSE.licenseType ? (a = parseFloat(Settings.LICENSE.expiration),
            a = new Date(a),
            this.remainingdays = this.daydiff(new Date, a),
  ...
  , Modal = Backbone.View.extend({
    ...
    call: function(a) {
        ...
        if (!$(a.target).hasClass("disabled_toolbar") && this[b])
            this[b](a);
        return !1
    },
    open: function() {
...
var MDXModal = Modal.extend({
  ...
  , SelectionsModal = Modal.extend({
    ...
    initialize: function(a) {
        ...
        "FILTER" == a.axis && (this.use_result_option = !1)) : (a.target.parents(".fields_list_body").hasClass("rows") && (this.axis = "ROWS"),
        a.target.parents(".fields_list_body").hasClass("columns") && (this.axis = "COLUMNS"),
        a.target.parents(".fields_list_body").hasClass("filter") && (this.axis = "FILTER",
        this.use_result_option = !1));
        ...
        void 0 != this.query.helper.model().parameters[b] && (this.paramvalue = this.query.helper.model().parameters[b].split(","))),
        $(this.el).find(".parameter").removeClass("hide"));
        b = $(this.el).find("#show_totals");
        b.val("");
        1 < _.size(a.levels) && c && c.hasOwnProperty("aggregators") && c.aggregators ? (0 < c.aggregators.length && (this.show_totals_option = c.aggregators[0]),
        b.removeAttr("disabled")) : (b.attr("disabled", !0),
    ...
    fetch_calcmembers_levels: function() {
        var a = this.member.hierarchy.split("].[")
          , b = !0;
        1 === a.length && (b = !1,
        a = this.member.hierarchy.split("."));
        if (1 < a.length)
            var c = a[1].replace(/[\[\]]/gi, "");
        a = a[0].replace(/[\[\]]/gi, "");
        this.workspace.block('\x3cspan class\x3d"processing_image"\x3e\x26nbsp;\x26nbsp;\x3c/span\x3e \x3cspan class\x3d"i18n"\x3e' + this.message + "\x3c/span\x3e ");
        b || (c = void 0 != c ? a + "." + c : a);
        (new Level({},{
            ui: this,
    ...
    get_levels: function(a, b) {
        b && 0 < b.length && (a.ui.topLevel = b[0],
        a.ui.get_members())
    },
    get_calcmembers: function() {
        ...
        this.topLevel.name === this.member.level ? _.filter(c, function(a) {
            a.hierarchyName === b && _.isEmpty(a.parentMember) && (a.uniqueName = a.hierarchyName + ".[" + a.name + "]",
            d.push(a))
        }) : _.filter(c, function(c) {
            c.hierarchyName === b && c.parentMemberLevel === a.member.level && (c.uniqueName = c.parentMember + ".[" + c.name + "]",
            d.push(c))
        });
        return d
    },
    ...
    populate: function(a, b) {
        ...
        var d = this.workspace.query.helper.getCalculatedMembers();
        if (0 < d.length)
            for (var e = this.get_calcmembers(), f = e.length, g = 0; g < f; g++) {
                var h = !1;
                d && 0 < d.length && _.each(d, function(a) {
                    a.uniqueName === e[g].uniqueName && (h = !0)
                });
        ...
        f = c.member.level;
        (d = c.workspace.query.helper.getHierarchy(c.member.hierarchy)) && d.levels.hasOwnProperty(f) && (this.selected_members = [],
        d.levels[f].selection && _.each(d.levels[f].selection.members, function(a) {
            var b = c.workspace.query.helper.getCalculatedMembers()
              , d = !1;
            b && 0 < b.length && _.each(b, function(b) {
        ...
        this.selection_type = d.levels[f].selection ? d.levels[f].selection.type : "INCLUSION");
        for (var k = [], d = 0, f = this.selected_members.length; d < f; d++)
            k.push(this.selected_members[d].obj.caption);
        0 == $(this.el).find(".used_selections .selection_options li.option_value").length && (d = $(this.el).find(".used_selections .selection_options"),
        d.empty(),
        f = _.template($("#template-selections-options").html())({
        ...
        0 < this.available_members.length && (d = $(this.el).find(".available_selections .selection_options"),
        d.empty(),
        f = _.template($("#template-selections-options").html())({
            ...
            source: function(a, b) {
                var d = 0 == c.show_unique_option ? "caption" : "name"
                  , e = $.map(c.available_members, function(b) {
                    ...
                    void 0 === e ? (e = b,
                    b = e[d]) : b = e.caption;
                    if (-1 < b.toLowerCase().indexOf(a.term.toLowerCase()))
                        return {
                            label: 0 == c.show_unique_option ? e.caption : e.uniqueName,
                            value: 0 == c.show_unique_option ? e.uniqueName : e.caption
                        }
                });
                b(e)
            },
        ...
        "EXCLUSION" === this.selection_type ? ($(this.el).find(".selection_type_inclusion").prop("checked", !1),
        $(this.el).find(".selection_type_exclusion").prop("checked", !0)) : ($(this.el).find(".selection_type_inclusion").prop("checked", !0),
        $(this.el).find(".selection_type_exclusion").prop("checked", !1));
        Saiku.i18n.translate();
        Saiku.ui.unblock()
    },
    ...
    click_move_selection: function(a, b) {
        a.preventDefault();
    ...
    show_unique_action: function() {
        ...
        !0 === this.show_unique_option ? ($(this.el).find(".available_selections, .used_selections").addClass("unique"),
        $(this.el).find(".available_selections, .used_selections").removeClass("caption")) : ($(this.el).find(".available_selections, .used_selections").addClass("caption"),
    ...
    save: function() {
        ...
        0 !== $(this.el).find(".used_selections input").length && (a.workspace.query.helper.removeAllLevelCalculatedMember(c),
        $(this.el).find(".used_selections .option_value input").each(function(b, f) {
            var g = $(f).val();
        ...
        var g = $("#parameter").val();
        b && b.levels.hasOwnProperty(d) && (b.levels[d].aggregators = [],
        f && b.levels[d].aggregators.push(f),
        f = $(a.el).find("input.selection_type:checked").val(),
        b.levels[d].selection = {
            type: f ? f : "INCLUSION",
            members: e
        },
        Settings.ALLOW_PARAMETERS && g && (b.levels[d].selection.parameterName = g,
        a.workspace.query.helper.model()));
        this.finished()
    },
    finished: function() {
  ...
  , DrillthroughModal = Modal.extend({
    ...
    initialize: function(a) {
        ...
        b && b.has("data") && (c = b.get("data").dimensions,
        d = b.get("data").measures);
        ...
        c = Saiku.session.sessionworkspace.cube[e].get("data").dimensions,
        d = Saiku.session.sessionworkspace.cube[e].get("data").measures);
    ...
    ok: function() {
        ...
        var a = $(this.el).find(".maxrows").val(), c;
        c = "?maxrows\x3d" + a + ("undefined" !== typeof this.position ? "\x26position\x3d" + this.position : "");
        c += "\x26returns\x3d" + b;
        "export" == this.action ? (a = Settings.REST_URL + "api/query/" + this.query.id + "/drillthrough/export/csv" + c,
        this.close(),
        window.open(a)) : "table" == this.action && (Saiku.ui.block("Executing drillthrough..."),
  ...
  , PermissionsModal = Modal.extend({
    ...
    initialize: function(a) {
        ...
        null !== a && "PRIVATE" == a && ($(this.el).find(".private_owner .owner").text(b),
        $(this.el).find(".private_owner").show());
        $(this.el).find(".i18n").i18n(Saiku.i18n.po_file)
    },
    add_role: function(a) {
        a.preventDefault();
        if ("PRIVATE" == this.acltype)
            return !1;
        ...
        0 < d ? (this.rolesacl[a] = b,
        $("\x3coption value\x3d'" + a + "'\x3e" + a + " [" + c + "]\x3c/option\x3e").appendTo($(this.el).find(".select_roles")),
        a = $(this.el).find(".filterbox").val("")) : alert("You need to chose at least one ACL method for this role."));
        return !1
    },
    keep_private: function(a) {
        $(this.el).find("input.private").is(":checked") ? ($(this.el).find(".permissions").addClass("disabled_toolbar"),
        $("input.acl, input.filterbox, input.add_role, input.remove_acl").prop("disabled", !0),
        this.acltype = "PRIVATE") : ($(this.el).find(".permissions").removeClass("disabled_toolbar"),
        $("input.acl, input.filterbox, input.add_role, input.remove_acl").prop("disabled", !1),
  ...
  , AboutModal = Modal.extend({
    ...
    render: function() {
        ...
        a = new Date(yourEpoch),
        $(this.el).find(".licenseexpr").text(a.toLocaleDateString()));
        void 0 != Settings.LICENSE.licenseType ? ($(this.el).find(".licensetype").text(Settings.LICENSE.licenseType),
        $(this.el).find(".licensename").text(Settings.LICENSE.name),
        $(this.el).find(".licenseuserlimit").text(Settings.LICENSE.userLimit),
        $(this.el).find(".licenseemail").text(Settings.LICENSE.email)) : $(this.el).find(".licensetype").text("Open Source License");
  ...
  , FilterModal = Modal.extend({
    ...
    save: function(a) {
        ...
        a = "";
        "undefined" != typeof this.expression && this.expression && "" !== this.expression ? ("Order" == this.expressionType ? (a = $("#fun").val(),
        this.success(a, this.expression)) : this.success(this.expression),
        this.close()) : (a += "You have to enter a MDX expression for the " + this.expressionType + " function! ",
        alert(a));
        return !1
    },
    error: function() {
  ...
  , CustomFilterModal = Modal.extend({
    ...
    initialize: function(a) {
        ...
        this.bind("open", function() {
            null !== b.func && ($(b.el).find(".function").val(b.func),
            b.switch_function({
                target: $(b.el).find(".function")
            }),
            $(b.el).find(".n").val(b.n),
            !0 === b.isMdx && null !== b.sortliteral && ($(this.el).find(".type").val("custom"),
            $(this.el).find(".sortingoption").html("").html("\x3ctextarea class\x3d'form-control sortliteral'\x3e" + b.sortliteral + "\x3c/textarea\x3e")))
        });
        if (isIE && 9 > isIE)
            $(this.el).find("form").on("submit", this.save)
    },
    ...
    switch_function: function(a) {
        a = $(a.target).val();
        "undefined" == typeof a || "" === a ? $(this.el).find(".filter_details").hide() : (a = a.replace("Top", "").replace("Bottom", ""),
        $(this.el).find(".ntype").html(a + ":"),
        $(this.el).find(".filter_details").show(),
        $(this.el).find(".sortingoption").html("").html(this.measure_list));
        return !1
    },
    switch_type: function(a) {
        "measure" == $(a.target).val() ? $(this.el).find(".sortingoption").html("").html(this.measure_list) : $(this.el).find(".sortingoption").html("").html("\x3ctextarea class\x3d'form-control sortliteral' /\x3e");
        return !1
    },
    save: function(a) {
        a.preventDefault();
        ...
        "" !== a ? alert(a) : (this.success(this.func, this.n, this.sortliteral),
        this.close());
        return !1
    },
    error: function() {
  ...
  , CalculatedMemberModal = Modal.extend({
    ...
    edit_cms: function(a) {
        ...
        _.each(a, function(a) {
            a.name === c.data("name") && (c.addClass("on"),
            b.$el.find("#cms-name").val(a.name),
            b.formulaEditor.setValue(a.formula),
            b.$el.find("#cms-dimension").val(a.hierarchyName),
            b.$el.find("#cms-pmember").val(a.parentMember),
            0 !== $('#cms-format option[value\x3d"' + a.properties.FORMAT_STRING + '"]').length || void 0 === a.properties.FORMAT_STRING && 0 === $('#cms-format option[value\x3d"' + a.properties.FORMAT_STRING + '"]').length ? (b.$el.find("#cms-format").val(a.properties.FORMAT_STRING),
            b.$el.find(".div-format-custom").hide()) : (b.$el.find("#cms-format").prop("selectedIndex", 1),
            b.$el.find(".div-format-custom").show(),
            b.$el.find("#cms-format-custom").val(a.properties.FORMAT_STRING)),
            b.pmUniqueName = a.parentMember || "",
            b.pmLevel = a.parentMemberLevel || "",
            b.lastLevel = a.previousLevel || "",
            b.pmBreadcrumbs = a.parentMemberBreadcrumbs || [],
            b.actualLevel = a.assignedLevel || "",
            b.type_dimension(),
            b.$el.find(".form-group-inline").data("action", "edit"),
            b.$el.find(".form-group-inline").data("oldcms", a.name))
        });
    ...
    show_del_cms: function(a) {
        ...
        (new WarningModal({
            ...
            message: '\x3cspan class\x3d"i18n"\x3eYou want to delete this\x3c/span\x3e ' + b + " \x3cb\x3e" + a.data("name") + "\x3c/b\x3e?",
            okay: this.del_cms,
            okayobj: this
        })).render().open();
    ...
    check_name_cms: function(a, b) {
        ...
        if (null === a || void 0 === a)
            var d = this.workspace.query.helper.getCalculatedMeasures()
              , e = this.workspace.query.helper.getCalculatedMembers()
              , c = []
              , c = c.concat(d, e);
        for (; 0 < c.length; )
            return c[0].name === b ? !0 : !1
    },
    ...
    type_dimension: function(a) {
        ...
        this.$el.find(".btn-clear-parent-member").removeAttr("disabled")) : (this.$el.find(".btn-parent-member").attr("disabled", "disabled"),
    ...
    openGrowthModal: function(a) {
        ...
        var b = Saiku.session.sessionworkspace.cube[this.workspace.selected_cube].get("data").measures;
        this.close();
        (new GrowthModal({
            workspace: this.workspace,
            measures: b,
    ...
    save: function(a) {
        ...
        h && (c.properties.FORMAT_STRING = h),
        "cad" === k ? (this.workspace.query.helper.addCalculatedMeasure(c),
        this.workspace.sync_query()) : (this.workspace.query.helper.editCalculatedMeasure(a, c),
        this.workspace.sync_query(),
        this.workspace.drop_zones.set_measures())) : (c = {
            name: b,
            dimension: f,
            uniqueName: "[" + e + "].[" + b + "]",
            caption: b,
        ...
        h && (c.properties.FORMAT_STRING = h),
        this.pmUniqueName && !_.isEmpty(this.pmUniqueName) && (c.parentMember = this.pmUniqueName,
        c.parentMemberLevel = this.pmLevel,
        c.previousLevel = this.lastLevel,
        c.parentMemberBreadcrumbs = this.pmBreadcrumbs,
        c.uniqueName = this.pmUniqueName + ".[" + b + "]",
        c.assignedLevel = this.actualLevel),
        this.workspace.query.helper.includeCmember(d, c.uniqueName),
        "cad" === k ? (this.workspace.query.helper.addCalculatedMember(c),
        this.workspace.sync_query()) : (this.workspace.query.helper.removeLevelCalculatedMember(d, "[" + e + "].[" + a + "]"),
        this.workspace.query.helper.editCalculatedMember(a, c),
        this.workspace.sync_query(),
    ...
    populate_function_list: function(a) {
        ...
        for (var b = "", c = 0; c < a.length; c++)
            b += '\x3coption title\x3d"' + a[c].description + '" data-desc\x3d"' + a[c].description + '" data-doc-link\x3d"' + a[c].doc_link + '" value\x3d"' + a[c].example + '"\x3e' + a[c].name + "\x3c/option\x3e";
    ...
    change_function_list: function(a) {
        ...
        $(a).data("doc-link") ? ($(".cms-doclink").prop("href", $(a).data("doc-link")),
        $(".cms-doclink").show()) : $(".cms-doclink").hide();
        if ($(a).val()) {
            var b = this.$el.find(".formula-editor").attr("id");
    ...
    open_select_member_selector: function(a) {
        ...
        this.$el.parents(".ui-dialog").find(".ui-dialog-title").text("Connection Details")) : "calcmember" === e && ((new ParentMemberSelectorModal({
            dialog: this,
            workspace: this.workspace,
            cube: this.workspace.selected_cube,
            dimensions: Saiku.session.sessionworkspace.cube[this.workspace.selected_cube].get("data").dimensions,
  ...
  , GrowthModal = Modal.extend({
    ...
    save: function(a) {
        ...
        a && (c.properties.FORMAT_STRING = a)),
        b.workspace.query.helper.addCalculatedMeasure(c),
    ...
    updateCalculatedMemberField: function() {
        ...
        if (this.asPercent)
            this.measureExpression = "( IIF( IsEmpty(" + b + "),NULL, ( " + a + " - (" + a + ", " + b + ")) / ( " + a + ", " + b + ")))";
        else if (this.asPercentAround100)
            this.measureExpression = "( IIF( IsEmpty(" + b + "),NULL, 1 + ( " + a + " - (" + a + ", " + b + ")) / ( " + a + ", " + b + ")))";
            ...
            this.measureExpression = "( IIF( IsEmpty(" + b + "),NULL, ( " + a + " - ( " + a + ", " + b + "))))";
    ...
    setAbsoluteOrRelative: function(a) {
        ...
        this.asPercentAlternative = this.asPercentAround100 = !1,
        ...
        $(this.el).find("#Dimensions").find("option:last").prop("disabled", !1)) : "asPercentAround100Checkbox" == a ? (this.asPercentAround100 = !this.asPercentAround100,
        this.asPercentAlternative = this.asPercent = !1,
        $(this.el).find("#Dimensions option").remove(),
        _.each(this.dimensions, function(a) {
            $(b.el).find("#Dimensions").append($("\x3coption\x3e\x3c/option\x3e").attr("value", a).text(a))
        }),
        $(this.el).find("#Dimensions").find("option:last").prop("disabled", !1)) : "asPercentAltCheckbox" == a && (this.asPercentAlternative = !this.asPercentAlternative,
        this.asPercentAround100 = this.asPercent = !1,
        $(this.el).find("#Dimensions option").remove(),
        a = this.workspace.query.helper.getAxis("ROWS").hierarchies,
        _.each(a, function(a) {
    ...
    calculate_nonempty_crossjoin: function(a) {
        var b;
        b = this.return_selected_members(a[a.length - 1]);
        b = "NONEMPTYCROSSJOIN(" + this.return_selected_members(a[a.length - 2]) + "," + b + ")";
        if (2 < a.length)
            for (var c = a.length - 2; 0 < c; --c)
                b = "NONEMPTYCROSSJOIN(" + this.return_selected_members(a[c - 1]) + "," + b + ")";
        return b
    },
    ...
    return_selected_members: function(a) {
        ...
        if (0 < c.selection.members.length)
            return _.each(c.selection.members, function(a) {
                b += a.uniqueName + ","
            }),
  ...
  , FormatAsPercentageModal = Modal.extend({
    ...
    save: function(a, b, c) {
        ...
        a.properties.FORMAT_STRING = "0.00%",
        this.workspace.query.helper.addCalculatedMeasure(a),
    ...
    formatMeasure: function(a, b) {
        var c = "";
        if ("formatOverRows" == b) {
            if (0 == this.selectedColumns.length)
            ...
            if (0 == this.selectedRows.length && 0 == this.selectedColumns.length)
    ...
    replaceMeasuresWithFormattedMeasures: function() {
        ...
        a.clearMeasures();
        for (var b = this.generatedMeasures, c = [], d = 0; d < b.length; d++)
            c[d] = {
                name: b[d].name,
                type: "CALCULATED"
            };
    ...
    setUserFeedback: function(a) {
        $("#userFeedback").html("\x3cp class\x3d'editor_info'\x3e" + a + "\x3c/p\x3e")
    },
  ...
  , ParentMemberSelectorModal = Modal.extend({
    ...
    initialize: function(a) {
        ...
        this.current_level = a.current_level;
        ...
        this.bind("open", function() {
            ...
            _.isEmpty(this.uniqueName) && _.isEmpty(this.breadcrumbs) && !_.isEmpty(this.dimension) ? (Saiku.ui.block('\x3cspan class\x3d"i18n"\x3eLoading...\x3c/span\x3e'),
            this.new_parent_member()) : _.isEmpty(this.dimension) || (Saiku.ui.block('\x3cspan class\x3d"i18n"\x3eLoading...\x3c/span\x3e'),
            this.edit_parent_member())
        })
    },
    new_parent_member: function() {
        if ("Measures" != this.dimension)
            (new Level({},{
                ui: this,
                cube: this.cube,
    ...
    populate_breadcrumbs: function(a) {
        for (var b = [], c = a.length, d = 0; d < c; d++)
            d !== a.length - 1 ? 0 === d || 1 === d ? b.push('\x3ca href\x3d"#fetch_crumb" class\x3d"crumb" data-position\x3d"' + d + '" data-action\x3d"false"\x3e' + a[d] + "\x3c/a\x3e \x26gt;") : b.push('\x3ca href\x3d"#fetch_crumb" class\x3d"crumb" data-position\x3d"' + d + '" data-action\x3d"true"\x3e' + a[d] + "\x3c/a\x3e \x26gt;") : b.push('\x3cspan class\x3d"last-crumb"\x3e' + a[d] + "\x3c/span\x3e");
        Saiku.ui.unblock();
    ...
    populate_members_list: function(a) {
        ...
        for (var d = 0; d < c; d++)
            ...
            b = $("\x3cli /\x3e").addClass("member").data("caption", a[d].caption).data("uniqueName", a[d].uniqueName).data("levelUniqueName", a[d].levelUniqueName ? a[d].levelUniqueName : !1).data("currentLevelUnique", a[d].levelUniqueName).html(a[d].name + "\x3cspan class\x3d'drill_member' style\x3d'float:right;'\x3eNext Level\x3c/span\x3e"),
            this.$el.find(".members-list").append(b);
        Saiku.ui.unblock()
    },
    get_levels: function(a, b) {
        var c;
        b ? (a.ui.breadcrumbs = [a.ui.dimension, a.ui.hierarchy, b[0].name],
        a.ui.get_last_level(),
        a.ui.populate_breadcrumbs(a.ui.breadcrumbs),
        a.ui.$el.find(".dialog_footer").find('a[href\x3d"#clear"]').data("name", b[0].name),
        c = new LevelMember({},{
            ui: a.ui,
            cube: a.ui.cube,
    ...
    get_child_members: function(a, b) {
        var c;
        b && 0 < b.length ? (a.ui.populate_members_list(b),
        c = b[0].levelUniqueName.split("].["),
        c = _.last(c).replace(/[\[\]]/gi, ""),
        a.ui.breadcrumbs.push(c),
        a.ui.breadcrumbs = _.uniq(a.ui.breadcrumbs),
        c = _.indexOf(a.ui.breadcrumbs, c),
        a.ui.breadcrumbs = _.initial(a.ui.breadcrumbs, a.ui.breadcrumbs.length - (c + 1)),
        a.ui.get_last_level(),
        a.ui.populate_breadcrumbs(a.ui.breadcrumbs)) : Saiku.ui.unblock()
    },
    return_actual_level: function(a, b) {
        b && 0 < b.length ? (a.ui.actualLevel = b[0].levelUniqueName,
        a.ui.current_level = _.last(a.ui.actualLevel.split("].[")).replace(/[\[\]]/gi, ""),
        console.log(this.current_level)) : Saiku.ui.unblock()
    },
    ...
    fetch_crumb: function(a) {
        ...
        this.reset_form(),
        this.breadcrumbs = _.initial(this.breadcrumbs, this.breadcrumbs.length - (Number(a.data("position")) + 1)),
    ...
    clear: function(a) {
        ...
        this.breadcrumbs = _.initial(this.breadcrumbs, this.breadcrumbs.length - (a + 1));
        this.populate_breadcrumbs(this.breadcrumbs)
    },
    save: function(a) {
        a.preventDefault();
        a = "";
        if ("undefined" === typeof this.uniqueName || _.isEmpty(this.uniqueName))
            a += "You have to choose a member for the calculated member!";
        if ("" !== a)
            alert(a);
        else {
            if ("parent-member-selector" === this.select_type) {
                ...
                "Measures" !== this.dimension ? (a = "[" + this.dimension + "].[" + this.hierarchy + "].",
                b = void 0 !== this.uniqueName.split(a)[1] ? this.uniqueName.split(a)[1] : this.uniqueName.split(a)[0],
                null != this.close_callback && this.close_callback(a + b)) : this.close_callback(this.uniqueName);
            this.$el.dialog("close")
        }
    },
  ...
  , DataSourcesModal = Modal.extend({
    ...
    populate_form: function(a) {
        var b = this.dataSources, c = b.length, d;
        for (d = 0; d < c; d++)
            b[d].name === a && (this.dialog.$el.find(this.formElements.url).val(b[d].url),
            this.dialog.$el.find(this.formElements.driver).val(b[d].driver),
            this.dialog.$el.find(this.formElements.username).val(b[d].username),
            this.dialog.$el.find(this.formElements.password).val(b[d].password))
    },
    add: function(a) {
        a.preventDefault();
  ...
  , QueryToolbar = Backbone.View.extend({
    ...
    activate_buttons: function(a) {
        ...
        a.data || $(this.el).find("a.export_button, a.stats").addClass("disabled_toolbar"),
        isIE && $(this.el).find("a.export_button").addClass("disabled_toolbar"))
    },
    template: function() {
    ...
    switch_render_button: function(a) {
        var b = $(a.target);
        a.preventDefault();
        if ($(a.target).hasClass("disabled_toolbar"))
            return !1;
        ...
        b.hasClass("render_chart") ? (Saiku.events.trigger("queryToolbar:render_chart", this, null ),
        "map" === a ? (b = (b = this.workspace.query.getProperty("saiku.ui.map.options")) ? b.mapDefinition.type : "",
        this.switch_render("map"),
        this.workspace.query.setProperty("saiku.ui.render.mode", "map"),
        this.workspace.query.setProperty("saiku.ui.render.type", b)) : (this.switch_render("chart"),
        this.workspace.query.setProperty("saiku.ui.render.mode", "chart"),
        b = 0 < $(this.el).find("ul.chart li a.on:first").size() ? $(this.el).find("ul.chart li a.on:first").attr("href").replace("#", "") : null ,
        null !== b && ("charteditor" === b && (b = $(this.el).find("ul.chart li").not(".chart_editor").find("a.on").attr("href").replace("#", "")),
        this.workspace.query.setProperty("saiku.ui.render.type", b)))) : (this.switch_render("table"),
    ...
    switch_render: function(a) {
        ...
        "chart" == a ? ($(this.el).find("ul.chart").show(),
        ...
        this.workspace.set_class_charteditor()) : "map" === a ? (this.$el.find("ul.renderer a.render_chart").addClass("on"),
        this.$el.find("ul.chart").show(),
        this.$el.find("ul.table").hide(),
        this.render_mode = "map",
        this.workspace.$el.find(".workspace_results").children().hide(),
        this.workspace.chart.$el.find(".canvas_wrapper").hide(),
        this.workspace.chart.show()) : ($(this.el).find("ul.chart").hide(),
        $(this.el).find("ul.table").show(),
        ...
        this.render_mode = "table",
        this.workspace.query.result.hasRun() && this.workspace.table.render({
    ...
    call: function(a) {
        ...
        if (!b.hasClass("disabled_toolbar")) {
            ...
            if ("table" == this.render_mode && this[c])
                this[c](a);
                ...
                if ("export_button" == c)
                    this.workspace.chart[c](a);
                else
                    this.workspace.chart.renderer.switch_chart(c),
                    this.workspace.query.setProperty("saiku.ui.render.type", c)
            } else if ("map" === this.render_mode && "map" !== c)
                if (this.workspace.chart.$el.find(".canvas_wrapper").find(".map-render").data("action", "querytoolbar"),
                b.hasClass("chartoption") && (d = {
                    mapDefinition: {}
                },
                this.workspace.query.setProperty("saiku.ui.map.options", d),
                this.workspace.query.setProperty("saiku.ui.render.mode", "chart"),
    ...
    spark_bar: function() {
        ...
        $(this.el).find("ul.table .spark_bar").hasClass("on") ? (this.spark_mode = "spark_bar",
        this.workspace.query.setProperty("saiku.ui.render.type", "spark_bar"),
        _.delay(this.render_row_viz, 10, "spark_bar")) : this.spark_mode = null
    },
    spark_line: function() {
        $(this.el).find("ul.table .spark_line").toggleClass("on");
        $(this.el).find("ul.table .spark_bar").removeClass("on");
        $(this.workspace.table.el).find("td.spark").remove();
        $(this.el).find("ul.table .spark_line").hasClass("on") ? (this.spark_mode = "spark_line",
        this.workspace.query.setProperty("saiku.ui.render.type", "spark_line"),
        _.delay(this.render_row_viz, 10, "spark_line")) : this.spark_mode = null
    },
    ...
    render_row_viz: function(a) {
        $(this.workspace.table.el).find("tr").each(function(b, c) {
            ...
            if (0 < d.length) {
                ...
                "spark_bar" == a ? f.add(pv.Bar).data(d).left(pv.Scale.linear(0, d.length).range(0, e).by(pv.index)).height(pv.Scale.linear(0, _.max(d)).range(0, 12)).width(6).bottom(0) : "spark_line" == a && (e /= 2,
                f.width(e),
                f.add(pv.Line).data(d).left(pv.Scale.linear(0, d.length - 1).range(0, e).by(pv.index)).bottom(pv.Scale.linear(d).range(0, 12)).strokeStyle("#000").lineWidth(1));
                f.render()
            }
        })
    }
  ...
  , WorkspaceToolbar = Backbone.View.extend({
    ...
    activate_buttons: function(a) {
        null !== a && a.data && a.data.cellset && 0 < a.data.cellset.length ? ($(a.workspace.toolbar.el).find(".button").removeClass("disabled_toolbar"),
        $(a.workspace.el).find("td.data").removeClass("cellhighlight").unbind("click"),
        $(a.workspace.el).find(".table_mode").removeClass("on")) : ($(a.workspace.toolbar.el).find(".button").addClass("disabled_toolbar").removeClass("on"),
        $(a.workspace.el).find(".fields_list .disabled_toolbar").removeClass("disabled_toolbar"),
        $(a.workspace.toolbar.el).find(".about, .new, .open, .save, .edit, .run,.auto,.non_empty,.toggle_fields,.toggle_sidebar,.switch_to_mdx, .mdx").removeClass("disabled_toolbar"));
    ...
    reflect_properties: function() {
        ...
        "VIEW" == Settings.MODE || this.workspace.isReadOnly ? ($(this.el).find("a.edit").hide(),
        $(this.el).find("a.save").hide()) : ("view" == this.workspace.viewState ? $(this.el).find("a.edit").removeClass("on") : $(this.el).find("a.edit").addClass("on"),
    ...
    save_query: function(a) {
        this.workspace.query && ("undefined" != typeof this.editor && (a = this.editor.getValue(),
    ...
    check_modes: function(a) {
        if ("undefined" !== typeof a && null !== a)
            if (0 < $(this.workspace.el).find(".workspace_results tbody.ui-selectable").length && $(this.workspace.el).find(".workspace_results tbody").selectable("destroy"),
            !$(a).hasClass("on"))
                $(this.workspace.el).find("td.data").removeClass("cellhighlight").unbind("click"),
                $(this.workspace.el).find(".table_mode").removeClass("on"),
                this.workspace.query.run();
            else if ($(a).hasClass("drillthrough_export"))
            ...
            else if ($(a).hasClass("drillthrough"))
            ...
            else if ($(a).hasClass("query_scenario"))
            ...
            else if ($(a).hasClass("drillacross"))
    ...
    export_xls: function(a) {
        void 0 != this.workspace.query.name ? (a = this.workspace.query.name.substring(this.workspace.query.name.lastIndexOf("/") + 1).slice(0, -5),
        window.location = Settings.REST_URL + this.workspace.query.url() + "/export/xls/" + this.workspace.query.getProperty("saiku.olap.result.formatter") + "?exportname\x3d" + encodeURIComponent(a) + "xls") : window.location = Settings.REST_URL + this.workspace.query.url() + "/export/xls/" + this.workspace.query.getProperty("saiku.olap.result.formatter")
    },
    export_csv: function(a) {
        void 0 != this.workspace.query.name ? (a = this.workspace.query.name.substring(this.workspace.query.name.lastIndexOf("/") + 1).slice(0, -6),
        window.location = Settings.REST_URL + this.workspace.query.url() + "/export/csv/" + this.workspace.query.getProperty("saiku.olap.result.formatter") + "?exportname\x3d" + encodeURIComponent(a)) : window.location = Settings.REST_URL + this.workspace.query.url() + "/export/csv/" + this.workspace.query.getProperty("saiku.olap.result.formatter")
    },
    export_pdf: function(a) {
        void 0 != this.workspace.query.name ? (a = this.workspace.query.name.substring(this.workspace.query.name.lastIndexOf("/") + 1).slice(0, -6),
        window.location = Settings.REST_URL + this.workspace.query.url() + "/export/pdf/" + this.workspace.query.getProperty("saiku.olap.result.formatter") + "?exportname\x3d" + encodeURIComponent(a)) : window.location = Settings.REST_URL + this.workspace.query.url() + "/export/pdf/" + this.workspace.query.getProperty("saiku.olap.result.formatter")
    },
    ...
    post_mdx_transform: function() {
        var a = this;
        "MDX" !== this.workspace.query.model.type && (this.workspace.query.model.queryModel = {},
        ...
        a.editor && (a.editor.setValue(b, 0),
        ...
        if (Settings.ALLOW_PARAMETERS) {
            ...
            ;
            a.editor && (a.editor.getSession().off("change", b),
            a.editor.getSession().on("change", b));
            a.workspace.update_parameters()
        }
    },
    run_mdx: function(a) {
  ...
  , WorkspaceDropZone = Backbone.View.extend({
    ...
    set_measures: function(a, b) {
        var c = [];
        $(this.el).find(".fields_list_body.details ul.connectable li.d_measure").each(function(a, b) {
            var f = {
    ...
    remove_dimension: function(a, b) {
        if ($(b.helper).hasClass("d_measure"))
            $(b.helper).detach(),
            this.set_measures();
        else {
        ...
        if (b.hasOwnProperty("queryModel") && b.queryModel.hasOwnProperty("axes")) {
            ...
            for (d in c) {
                ...
                _.each(c[d].hierarchies, function(b) {
                    ...
                    for (var d in b.levels)
                        c.find('li a[level\x3d"' + d + '"]').parent().show(),
                        $(a.workspace.dimension_list.el).find('ul.d_hierarchy[hierarchy\x3d"' + b.name + '"] li a[level\x3d"' + d + '"]').parent().draggable("disable").parents(".parent_dimension").find(".folder_collapsed").addClass("selected");
    ...
    select_dimension: function(a, b) {
        ...
        if ($(b.item).is(":visible")) {
            ...
            var e = b.item.parents(".axis_fields").parent().attr("title"), f = $(a.target).parents(".axis_fields").parent().attr("title"), g = b.item.hasClass("d_level"), h = b.item.hasClass("dimension-level-calcmember"), k;
            h || (g ? (k = b.item.find("a.level").attr("level"),
            this.workspace.query.helper.includeLevel(e, c, k, d)) : this.workspace.query.helper.moveHierarchy(f, e, c, d));
            $(b.item).detach();
            this.workspace.sync_query();
            this.workspace.query.run();
    ...
    find_type_time: function(a, b, c) {
        ...
        if (void 0 === d.hierarchies || null === d.hierarchies)
            d.hierarchies = _.findWhere(d.dimensions.hierarchies, {
                name: a + "." + b
            });
        ...
        if (null === d.level || void 0 === d.level)
            d.level = _.findWhere(d.hierarchies.levels, {
                caption: c
            });
        return d
    ...
    selections: function(a, b) {
        ...
        var l = decodeURIComponent(this.member.hierarchy), l = this.workspace.query.helper.getHierarchy(l), m;
    ...
    measure_action: function(a) {
        var b = this;
        if ("undefined" == typeof this.workspace.query || "QUERYMODEL" != this.workspace.query.model.type || "view" == Settings.MODE)
            return !1;
            ...
            build: function(a, e) {
                ...
                return {
                    callback: function(a, b) {
                        ...
                        if ("cancel" !== a) {
                            if ("reset" === a)
                                c.location = SaikuOlapQueryTemplate.queryModel.details.location,
    ...
    limit_axis: function(a) {
        ...
        $.contextMenu({
            ...
            ignoreRightClick: !0,
            build: function(a, f) {
                var g = {}, h = Saiku.session.sessionworkspace.cube[b.workspace.selected_cube].get("data").measures, k = b.workspace.query.helper.getAxis(d), l = k.hierarchies, m, n, r, q, s, t, x, z, y, v = !1, B = !1, C = !1;
                k && k.filters && _.each(k.filters, function(a) {
                    "N" == a.flavour && (m = k["function"],
                    n = a.expressions[0],
                    r = a.expressions[1],
                    C = !0);
                    "Generic" == a.flavour && (q = a.expressions[0],
                    v = !0)
                });
                k && k.sortOrder && (s = k.sortOrder,
                t = k.sortEvaluationLiteral,
                B = !0);
                k && k.aggregators && 0 < k.aggregators.length && (y = k.aggregators[0]);
                null !== m && null === r ? z = m + "###SEPARATOR###" + n : null !== m && null !== r && null !== n && (z = "custom");
                B && null !== s && (x = "customsort");
                _.each(h, function(a) {
                    g[a.uniqueName] = {
                        name: a.caption,
                        payload: {
                ...
                _.each(l, function(a) {
                    _.each(a.levels, function(b) {
                        g[a.name] = {
                            name: a.caption,
                            payload: {
                                n: 10,
                                sortliteral: a.name + ".[" + b.name + "].CURRENTMEMBER.ORDERKEY"
                            }
                        }
                    })
                });
                ...
                l = function(a, b) {
                    var c = {}, d;
                    for (d in a)
                        c[b + "###SEPARATOR###" + d] = _.clone(a[d]),
                        c[b + "###SEPARATOR###" + d].fun = b,
                        b == m && r == d && a[d].payload.n == n && (z = b + "Quick",
                        c[b + "###SEPARATOR###" + d].name = "\x3cb\x3e" + a[d].name + "\x3c/b\x3e"),
                        b == s && t == d && (x = b + "Quick",
                        c[b + "###SEPARATOR###" + d].name = "\x3cb\x3e" + a[d].name + "\x3c/b\x3e");
                    return c
                }
                ...
                var u = l.grand_totals.items;
                if (y)
                    for (var G in u)
                        G.substring(12) == y && (u[G].name = "\x3cb\x3e" + u[G].name + "\x3c/b");
                else
                    u.show_totals_not.name = "\x3cb\x3e" + u.show_totals_not.name + "\x3c/b";
                g["10"] = {
                    payload: {
                        n: 10
                    }
                ...
                v && (y = l.filter,
                y.name = "\x3cb\x3e" + y.name + "\x3c/b\x3e",
                y.items.customfilter.name = "\x3cb\x3e" + y.items.customfilter.name + "\x3c/b\x3e");
                B && (B = l.sort.items,
                l.sort.name = "\x3cb\x3e" + l.sort.name + "\x3c/b\x3e",
                x in B && (B[x].name = "\x3cb\x3e" + B[x].name + "\x3c/b\x3e"));
                C && (B = l.limit.items,
                l.limit.name = "\x3cb\x3e" + l.limit.name + "\x3c/b\x3e",
                z in B && (B[z].name = "\x3cb\x3e" + B[z].name + "\x3c/b\x3e"));
                return {
                    callback: function(a, e) {
                        ...
                        if ("cancel" != a)
                            if ("clearfilter" == a)
                            ...
                            else if ("stringfilter" == a)
                                f = function(a, c, d) {
                                    c = [];
                                    c.push(a + ('.CurrentMember.Name MATCHES ("(?i).*' + d + '.*")'));
                                    b.workspace.query.helper.removeFilter(k, "Generic");
                                    k.filters.push({
                                        flavour: "Generic",
                                        operator: null ,
                                ...
                                if ("clearsort" == a)
                                    ...
                                    var v = a.split("###SEPARATOR###")[1];
                                    -1 < _.indexOf(["ASC", "BASC", "DESC", "BDESC"], f) ? (k.sortOrder = f,
                                    k.sortEvaluationLiteral = g[v].payload.sortliteral) : -1 < _.indexOf(["Param"], f) ? (console.log("here"),
                                    k.sortEvaluationLiteral = g[v].payload.sortliteral) : (l = [],
                                    l.push(g[v].payload.n),
                                    (v = g[v].payload.sortliteral) && l.push(v),
                                    b.workspace.query.helper.removeFilter(k, "N"),
                                    k.filters.push({
                                        flavour: "N",
                                        operator: null ,
  ...
  , Table = Backbone.View.extend({
    ...
    clicked_cell: function(a) {
        ...
        $.contextMenu({
            ...
            build: function(a, d) {
                ...
                h.get("catalog");
                h.get("cube");
                var l = k.properties.dimension, m = k.properties.hierarchy, n = k.properties.level, r = "", q = JSON.stringify({
                    hierarchy: m,
                    uniquename: n,
                    type: "level",
                    action: "delete"
                    ...
                    type: "member",
                    action: "add"
                }), h = k.properties.uniquename, s = {}, g = b.workspace.selected_cube, t;
                Saiku.session.sessionworkspace.cube[g] && t && measures || ("undefined" !== typeof localStorage && localStorage && null !== localStorage.getItem("cube." + g) ? Saiku.session.sessionworkspace.cube[g] = new Cube(JSON.parse(localStorage.getItem("cube." + g))) : (Saiku.session.sessionworkspace.cube[g] = new Cube({
                    key: g
                }),
                Saiku.session.sessionworkspace.cube[g].fetch({
                    async: !1
                })),
                t = Saiku.session.sessionworkspace.cube[g].get("data").dimensions);
                var x = []
                ...
                _.each(t, function(a) {
                    a.name == l && _.each(a.hierarchies, function(a) {
                        a.uniqueName == m && _.each(a.levels, function(a) {
                            ...
                            -1 < _.indexOf(x, a.uniqueName) && (s[a.name].disabled = !0,
                            s["remove-" + a.name] = {
                                name: a.caption,
                                payload: JSON.stringify({
                                    hierarchy: m,
                            ...
                            a.uniqueName == n && (r = a.caption,
                            l_name = a.name);
                            s["keep-" + a.name] = s[a.name];
                            s["include-" + a.name] = JSON.parse(JSON.stringify(s[a.name]));
                            s["keep-" + a.name].payload = q + "," + s[a.name].payload
                        })
                    })
                });
                s.keeponly = {
                ...
                s.hasOwnProperty("remove-" + l_name) && s.hasOwnProperty("include-" + l_name) && (s.showall = {
                    payload: s["remove-" + l_name].payload + ", " + s["include-" + l_name].payload
                });
                t = function(a) {
                    var b = {}, c;
                    for (c in s)
                        null !== a && a.length < c.length && c.substr(0, a.length) == a && (b[c] = s[c]);
                    return b
                }
                ;
                h = {
                ...
                return {
                    callback: function(a, c) {
                        ...
                        if ("keeponly" === a) {
                            ...
                            g && g.levels.hasOwnProperty(r) && (d.push({
                                uniqueName: k.properties.uniquename,
                                caption: k.properties.uniquename
                            }),
                            g.levels[r].selection = {
                                type: "INCLUSION",
                                members: d
                            },
                            b.workspace.drop_zones.synchronize_query(),
                            b.workspace.query.run(!0))
                        } else if ("filterlevel" === a)
                            d = k.properties.level.substring(k.properties.level.lastIndexOf(".") + 1),
                            d = d.replace("[", "").replace("]", ""),
                            (new SelectionsModal({
                                target: e,
                                name: "Filter Level",
                            ...
                            "keep" === a.substring(0, a.indexOf("-")) ? (h = a.substring(a.indexOf("-") + 1),
                            (g = b.workspace.query.helper.getHierarchy(m)) && g.levels.hasOwnProperty(r) && (d.push({
                                uniqueName: k.properties.uniquename,
                                caption: k.properties.uniquename
                            }),
                            g.levels[r].selection = {
                            ...
                            b.workspace.query.helper.includeLevel(f, m, h, null ),
                            b.workspace.drop_zones.synchronize_query(),
                            b.workspace.query.run(!0))) : "include" === a.substring(0, a.indexOf("-")) && (h = a.substring(a.indexOf("-") + 1),
                            b.workspace.query.helper.includeLevel(f, m, h, null ),
                            b.workspace.drop_zones.synchronize_query(),
                            b.workspace.query.run(!0))
                    },
    ...
    render: function(a, b) {
        "undefined" == typeof a || "undefined" == typeof a.data || $(this.workspace.el).is(":visible") && !$(this.el).is(":visible") || null !== a.data && null !== a.data.error || null === a.data || a.data.height && 0 === a.data.height || (this.clearOut(),
        $(this.el).html("Rendering " + a.data.width + " columns and " + a.data.height + " rows..."),
        _.delay(this.process_data, 2, a.data))
    },
    clearOut: function() {
        this.renderer.clear();
        $(this.workspace.el).find(".workspace_results").unbind("scroll");
        var a = document.getElementById(this.id);
        null == a && (this.workspace.tab.select(),
  ...
  , Workspace = Backbone.View.extend({
    ...
    initialize: function(a) {
        _.bindAll(this, "caption", "adjust", "toggle_sidebar", "prepare", "new_query", "set_class_charteditor", "init_query", "update_caption", "populate_selections", "refresh", "sync_query", "cancel", "cancelled", "no_results", "error", "switch_view_state");
        _.extend(this, Backbone.Events);
        this.loaded = !1;
        ...
        a && a.item && (this.item = a.item) && this.item.hasOwnProperty("acl") && 0 > _.indexOf(this.item.acl, "WRITE") && (this.isReadOnly = !0,
        this.viewState = "view");
        ...
        this.query.save({}, {
            ...
            error: function() {
                ...
                if (1 > $("body").find(".error_loading_query").length) {
                    ...
                    a || ($('\x3cspan class\x3d"i18n error_loading_query"\x3eError Loading Query\x3c/span\x3e').hide().appendTo("body"),
                    Saiku.i18n.translate(),
                    a = $(".error_loading_query").text())
                } else
                    a = $(".error_loading_query").text();
                alert(a)
            }
        ...
        void 0 != a && void 0 != a.processURI && 0 == a.processURI && (b = {});
    ...
    caption: function(a) {
        ...
        return "\x3cspan class\x3d'i18n'\x3eUnsaved query\x3c/span\x3e (" + Saiku.tabs.queryCount + ")"
    },
    ...
    template: function() {
        var a = $("#template-workspace").html() || ""
          , b = !1;
        this.isUrlCubeNavigation && (b = this.selected_cube.split("/")[3],
        b = this.selected_cube_template(b));
        return _.template(a)({
    ...
    render: function() {
        ...
        this.isReadOnly || Settings.MODE && ("view" == Settings.MODE || "table" == Settings.MODE || "map" == Settings.MODE || "chart" == Settings.MODE) ? ($(this.el).find(".workspace_editor").remove(),
        this.toggle_sidebar(),
        ...
        $(this.toolbar.el).find(".run, .auto, .toggle_fields, .toggle_sidebar,.switch_to_mdx, .new").parent().remove()) : ($(this.el).find(".workspace_editor").append($(this.drop_zones.el)),
        ...
        !Settings.MODE || "table" != Settings.MODE && "chart" != Settings.MODE && "map" != Settings.MODE ? ($(this.el).find(".workspace_toolbar").append($(this.toolbar.el)),
        $(this.el).find(".query_toolbar").append($(this.querytoolbar.el)),
        $(this.el).find(".upgrade").append($(this.upgrade.el))) : ($(this.el).find(".workspace_toolbar").remove(),
        ...
        Saiku.session.isAdmin || !1 !== Settings.SHOW_REFRESH_NONADMIN || $(this.el).find(".refresh_cubes_nav").hide();
        return this
    },
    clear: function() {
        this.table.clearOut();
    ...
    adjust: function() {
        var a = $(this.el).find(".sidebar_separator")
          , b = 87;
        if (!0 === Settings.PLUGIN || !0 === Settings.BIPLUGIN)
            b = 2,
            "table" == Settings.MODE && (b = -5);
        if (0 === $("#header").length || $("#header").is("hidden"))
            b = 2;
        ...
        var a = $(this.el).find(".workspace_editor").is(":hidden") ? 0 : $(this.el).find(".workspace_editor").height()
          , c = $(this.el).find(".query_processing").is(":hidden") ? 0 : $(this.el).find(".query_processing").height() + 62
        ...
        $(this.el).find(".workspace_fields").css({
            height: $("body").height() - b - $(this.el).find(".workspace_toolbar").height() - d - 20
        });
                ...
                _.each(d.schemas, function(e) {
                    0 < e.cubes.length && _.each(e.cubes, function(f) {
                        if ("undefined" === typeof f.visible || f.visible) {
                            ...
                            a.schema === g && a.cube === f && (b.selected_cube = c.name + "/" + d.name + "/" + g + "/" + f,
                            b.isUrlCubeNavigation = !0,
                            b.paramsURI = a,
                            _.delay(b.new_query, 1E3))
                        }
                    })
                })
            })
    ...
    create_new_query: function(a) {
        a.query && (a.query.destroy(),
        a.query.clear(),
        a.query.name && (a.query.name = void 0,
        a.update_caption(!0)),
        a.query.name = void 0);
        a.clear();
        a.processing.hide();
        ...
        if (!a.selected_cube)
            return $(a.el).find(".calculated_measures, .addMeasure").hide(),
            $(a.el).find(".dimension_tree").html(""),
            $(a.el).find(".measure_tree").html(""),
            !1;
        a.metadata = Saiku.session.sessionworkspace.cube[a.selected_cube];
        for (var b = a.selected_cube.split("/"), c = b[3], d = 4, e = b.length; d < e; d++)
            c += "/" + b[d];
    ...
    init_query: function(a) {
        ...
        try {
            ...
            "chart" == d ? (!Settings.MODE || "chart" !== Settings.MODE || "map_heat" !== e && "map_geo" !== e && "map_marker" !== e || (this.query.setProperty("saiku.ui.render.mode", "chart"),
            e = "stackedBar"),
            $(this.chart.el).find(".canvas_wrapper").hide(),
            this.chart.renderer.switch_chart(e),
            $(this.querytoolbar.el).find('ul.chart [href\x3d"#' + e + '"]').parent().siblings().find(".on").removeClass("on"),
            $(this.querytoolbar.el).find('ul.chart [href\x3d"#' + e + '"]').addClass("on"),
            this.set_class_charteditor()) : "table" == d && e in this.querytoolbar ? (this.querytoolbar.render_mode = "table",
            this.querytoolbar.spark_mode = e,
            $(this.querytoolbar.el).find("ul.table a." + e).addClass("on")) : "map" === d && (this.querytoolbar.$el.find("ul.chart \x3e li").find("a").removeClass("on"),
            this.querytoolbar.$el.find('ul.chart [href\x3d"#' + d + '"]').addClass("on"))
        } catch (f) {
            Saiku.error(this.cid, f)
        }
        "table" == Settings.MODE && this.query ? this.query.run(!0) : ("MDX" == this.query.model.type ? (this.query.setProperty("saiku.olap.result.formatter", "flat"),
        $(this.el).find(".sidebar").hasClass("hide") || this.toggle_sidebar(),
        $(this.el).find(".workspace_fields").addClass("hide"),
        this.toolbar.switch_to_mdx()) : ($(this.el).find(".workspace_editor").removeClass("hide").show(),
        ...
        this.adjust(),
        this.switch_view_state(this.viewState, !0),
        $(this.el).find(".sidebar").hasClass("hide") || "chart" != Settings.MODE && "table" != Settings.MODE && "map" != Settings.MODE && "view" != Settings.MODE && !this.isReadOnly || this.toggle_sidebar(),
        "view" == Settings.MODE && this.query || this.isReadOnly ? (this.query.run(!0),
        void 0 === this.selected_cube && (c = this.query.model.cube.schema,
        this.selected_cube = this.query.model.cube.connection + "/" + this.query.model.cube.catalog + "/" + ("" === c || null === c ? "null" : c) + "/" + encodeURIComponent(this.query.model.cube.name),
        $(this.el).find(".cubes").val(this.selected_cube))) : (void 0 === this.selected_cube && (c = this.query.model.cube.schema,
        this.selected_cube = this.query.model.cube.connection + "/" + this.query.model.cube.catalog + "/" + ("" === c || null === c ? "null" : c) + "/" + encodeURIComponent(this.query.model.cube.name),
        $(this.el).find(".cubes").val(this.selected_cube)),
        this.selected_cube ? (c = Saiku.session.sessionworkspace.cube[this.selected_cube],
        ...
        "undefined" != typeof a && this.query.run(!0),
        Saiku.i18n.translate()))
    },
    set_class_charteditor: function() {
        this.query.getProperty("saiku.ui.chart.options") && $(this.querytoolbar.el).find('ul.chart [href\x3d"#charteditor"]').addClass("on")
    },
    synchronize_query: function() {
        this.isReadOnly || Settings.hasOwnProperty("MODE")
    },
    sync_query: function(a) {
        if ("QUERYMODEL" === this.query.helper.model().type && (a = a ? a : $(this.dimension_list.el),
        !this.isReadOnly && (!Settings.hasOwnProperty("MODE") || "table" != Settings.MODE && "view" != Settings.MODE))) {
            ...
            a.find(".calculated_measures").html(b),
            a.find(".calculated_measures").find(".measure").parent("li").draggable({
                cancel: ".not-draggable",
    ...
    update_parameters: function() {
        ...
        if (a.hasOwnProperty("query") && Settings.ALLOW_PARAMETERS && "view" !== Settings.MODE && "view" !== a.viewState) {
            var b = "\x3cspan class\x3d'i18n'\x3eParameters\x3c/span\x3e: ", c = this.query.helper.model().parameters, d = !1, e;
            for (e in c) {
                var f = "", g;
                c[e] && null !== c[e] && (f = c[e],
                g = c[e].split(","));
    ...
    render_result: function(a) {
        $(this.el).find(".workspace_results_info").empty();
        if (null !== a.data && null !== a.data.error)
            return this.error(a);
        if (null === a.data || a.data.cellset && 0 === a.data.cellset.length)
        ...
        void 0 != a && (a = JSON.parse(a),
        b = "",
        null != a.title && "" != a.title && (b = '\x3ch3\x3e\x3cspan class\x3d"i18n"\x3eTitle:\x3c/span\x3e\x3c/h3\x3e \x26nbsp;' + a.title + "\x3cbr/\x3e"),
        null != a.variable && "" != a.variable && (b += '\x3ch3\x3e\x3cspan class\x3d"i18n"\x3eVariable:\x3c/span\x3e\x3c/h3\x3e \x26nbsp;' + a.variable + "\x3cbr/\x3e"),
        null != a.explanation && "" != a.explanation && (b += '\x3ch3\x3e\x3cspan class\x3d"i18n"\x3eExplanation:\x3c/span\x3e\x3c/h3\x3e \x26nbsp;' + a.explanation),
        $(this.el).find(".workspace_results_titles").html(b));
        this.adjust();
        Saiku.i18n.translate()
    },
    switch_view_state: function(a, b) {
        var c = a || "edit";
        "edit" == c ? (this.toolbar.toggle_fields_action("show", b),
        this.query && "MDX" == this.query.get("type") && this.toolbar.editor.gotoLine(0),
        $(this.el).find(".sidebar").hasClass("hide") && this.toggle_sidebar(),
        $(this.toolbar.el).find(".auto, .toggle_fields, .toggle_sidebar,.switch_to_mdx, .new").parent().css({
            display: "block"
        })) : "view" == c && (this.toolbar.toggle_fields_action("hide", b),
        $(this.el).find(".sidebar").hasClass("hide") || this.toggle_sidebar(),
  ...
  , MoveRepositoryObject = Modal.extend({
    ...
    populate: function(a) {
        function b(a) {
            _.forEach(a, function(a) {
                "FOLDER" === a.type && (c.queries[a.path] = a,
                b(a.repoObjects))
            })
        }
        var c = this;
    ...
    toggle_folder: function(a) {
        ...
        c.removeClass("hide")) : (b.children(".folder_row").find(".sprite").addClass("collapsed"),
        c.addClass("hide"));
        this.select_folder();
        this.select_name(a);
        return !1
  ...
  , OpenQuery = Backbone.View.extend({
            ...
            events: {
                show: function(b) {
                    ...
                    var d = $(this).find("a").attr("href").replace("#", "")
                      , d = a.queries[d];
                    "undefined" != typeof d.acl && 0 > _.indexOf(d.acl, "WRITE") ? (b.commands["delete"].disabled = !0,
                    b.items["delete"].disabled = !0,
                    b.commands.edit.disabled = !0,
                    b.items.edit.disabled = !0,
                    b.commands.move.disabled = !0,
                    b.items.move.disabled = !0) : (b.commands["delete"].disabled = !1,
                    b.items["delete"].disabled = !1,
                    b.commands.edit.disabled = !1,
                    b.items.edit.disabled = !1,
                    b.commands.move.disabled = !1,
                    b.items.move.disabled = !1);
                    $(this).hasClass("folder_row") ? (b.commands.open.disabled = !0,
                    b.items.open.disabled = !0) : (b.commands.open.disabled = !1,
                    b.items.open.disabled = !1)
                }
            },
    ...
    search_file: function(a) {
        ...
        "undefined" == typeof b || "" === b || null === b || 27 == a.which || 9 == a.which ? this.cancel_search() : ($(this.el).find(".search_file").val() ? $(this.el).find(".cancel_search").show() : $(this.el).find(".cancel_search").hide(),
        $(this.el).find("li.query").removeClass("hide"),
        $(this.el).find("li.query a").each(function(a) {
    ...
    view_query: function(a) {
        ...
        "undefined" != typeof c.acl && -1 < _.indexOf(c.acl, "WRITE") && ($(this.el).find(".for_queries .delete").parent().removeClass("hide"),
        ...
        try {
            ...
            if (1 < d.length) {
                ...
                1 == d.length && $(this.el).find(".add_folder").parent().removeClass("hide")
        } catch (g) {}
        var d = $(this.el).find(".workspace_results").html("\x3ch3\x3e\x3cstrong\x3e" + c.name + "\x3c/strong\x3e\x3c/h3\x3e"), d = $('\x3cul id\x3d"query_info" /\x3e').appendTo(d), h;
        for (h in c)
            c.hasOwnProperty(h) && "name" != h && d.append($("\x3cli /\x3e").html("\x3cstrong\x3e" + h + "\x3c/strong\x3e : " + c[h]));
    ...
    view_folder: function(a) {
        ...
        $(this.el).find(".for_queries").addClass("hide");
        $(this.el).find(".for_folder").addClass("hide");
        "undefined" != typeof c.acl && -1 < _.indexOf(c.acl, "WRITE") && ($(this.el).find(".for_folder .delete").parent().removeClass("hide"),
    ...
    add_folder: function(a) {
        ...
        if ("undefined" !== typeof $selected && $selected)
            if ($selected.hasClass("folder_row"))
                a = $selected.children("a").attr("href"),
                a = 1 < a.length ? a.substring(1, a.length) : a,
                a += "/";
    ...
    toggle_folder: function(a) {
        ...
        b.children(".folder_row").find(".sprite").hasClass("collapsed") ? (b.children(".folder_row").find(".sprite").removeClass("collapsed"),
        d.removeClass("hide"),
        Settings.REPOSITORY_LAZY && this.fetch_lazyload(b, c)) : (b.children(".folder_row").find(".sprite").addClass("collapsed"),
        d.addClass("hide"),
        Settings.REPOSITORY_LAZY && b.find(".folder_content").remove());
        this.view_folder(a);
        return !1
    },
    ...
    open_contents: function(a) {
        ...
        (new WarningModal({
            ...
            message: '\x3cspan class\x3d"i18n"\x3eYou are about to open\x3c/span\x3e ' + b.length + ' \x3cspan class\x3d"i18n"\x3equeries\x3c/span\x3e',
  ...
  , SaveQuery = Modal.extend({
    ...
    toggle_folder: function(a) {
        ...
        c.removeClass("hide"),
        Settings.REPOSITORY_LAZY && this.fetch_lazyload(a, b)) : (a.children(".folder_row").find(".sprite").addClass("collapsed"),
        c.addClass("hide"),
        Settings.REPOSITORY_LAZY && a.find(".folder_content").remove());
    ...
    search_file: function(a) {
        ...
        "undefined" == typeof b || "" === b || null === b || 27 == a.which || 9 == a.which ? this.cancel_search() : ($(this.el).find(".search_file").val() ? $(this.el).find(".cancel_search").show() : $(this.el).find(".cancel_search").hide(),
        $(this.el).find("li.query").removeClass("hide"),
        $(this.el).find("li.query a").filter(function(a) {
    ...
    save: function(a) {
        ...
        null !== this.folder_name && void 0 !== this.folder_name && 0 < this.folder_name.length ? null !== c && 0 < c.length ? this.repository.fetch({
            success: function(d, e) {
                ...
                if (-1 < f.indexOf(c) && b.query.get("name") != c)
                    (new OverwriteModal({
                        name: c,
                        foldername: "",
                        parent: b
                    ...
                    b.query.trigger("query:save"),
                    b.copy_to_repository(),
                    a.stopPropagation(),
                    a.preventDefault(),
                    !1
            }
  ...
  , OpenDialog = Modal.extend({
    ...
    export_zip: function(a) {
        ...
        if ("undefined" != typeof a && "" !== a) {
            var b = Settings.REST_URL + (new RepositoryZipExport({
                directory: a
            })).url();
  ...
  , Tab = Backbone.View.extend({
    ...
    render: function() {
        ...
        this.content.render();
        $(this.el).html(this.template());
        !1 === this.close && ($(this.el).find(".close_tab").hide(),
        ...
        $.contextMenu({
            ...
            callback: function(b, d) {
                ...
                a.select()) : "closeothers" == b ? (e.select(),
                Saiku.tabs.close_others(e)) : "duplicate" == b ? Saiku.tabs.duplicate(e) : "new" == b && Saiku.tabs.new_tab()
            },
            items: b
        });
        return this
  ...
  , TabPager = Backbone.View.extend({
    ...
    render: function() {
        for (var a = "", b = 0, c = this.tabset._tabs.length; b < c; b++)
            a += "\x3ca href\x3d'#" + b + "'\x3e" + this.tabset._tabs[b].caption + "\x3c/a\x3e\x3cbr /\x3e";
        $(this.el).html(a);
        $(this.el).find(".i18n").i18n(Saiku.i18n.po_file)
    },
    select: function(a) {
  ...
  , TabSet = Backbone.View.extend({
    ...
    find: function(a) {
        for (var b = 0, c = this._tabs.length; b < c; b++)
            if (this._tabs[b].id == a)
                return this._tabs[b];
        return null
    },
    select: function(a) {
    ...
    remove: function(a) {
        for (var b = 0, c = this._tabs.length; b < c; b++)
            this._tabs[b] == a && (this._tabs.splice(b, 1),
            Saiku.session.trigger("tab:remove", {
                tab: a
            }),
            this.pager.render(),
            this._tabs[this._tabs[b] ? b : this._tabs.length - 1].select());
        return !0
    },
    close_others: function(a) {
        var b = _.indexOf(this._tabs, a);
        this._tabs[b].select();
        for (b = 0; 1 < this._tabs.length; )
            this._tabs[b] != a ? this._tabs[b].remove() : b++
    },
    close_all: function() {
        for (var a = 0, b = this._tabs.length; a < b; a++)
            this._tabs[a].remove()
    },
  ...
  , RepositoryObject = Backbone.Model.extend({
    ...
    parse: function(a) {
        if (this.dialog)
            return this.dialog.generate_grids_reports(a),
            a
    },
    url: function() {
        return this.file ? repoPathUrl() + "/resource?file\x3d" + this.file : repoPathUrl() + "/resource"
    }
})
  ...
  , SavedQuery = Backbone.Model.extend({
    ...
    move_query_to_workspace: function(a, b) {
        var c = b, d = a.get("file"), e;
        for (e in Settings)
            if ("PARAM" == e.match("^PARAM"))
                var f = e.substring(5, e.length)
                  , g = new RegExp("\\$\\{" + f + "\\}","g")
  ...
  , Repository = Backbone.Collection.extend({
    ...
    url: function() {
        var a = repoPathUrl() + "?type\x3d" + (this.type ? this.type : "saiku,sdb");
  ...
  , QueryAction = Backbone.Model.extend({
    ...
    handle: function(a, b, c) {
        ...
        "save" == a ? this.save({}, c) : "delete" == a ? (this.set("id", this.id),
        this.destroy(c)) : "fetch" == a && (this.parse = function() {}
        ,
        this.fetch(c))
    }
})
  , QueryScenario = Backbone.Model.extend({
    ...
    attach_listeners: function(a) {
        a.workspace.query && a.workspace.query.properties && "true" === a.workspace.query.properties.properties["org.saiku.connection.scenario"] && $(a.workspace.el).find(".query_scenario").hasClass("on") && $(a.workspace.el).find("td.data").click(this.clicked_cell)
    },
    clicked_cell: function(a) {
  ...
  , Query = Backbone.Model.extend({
    ...
    run: function(a, b) {
        ...
        if ("undefined" == typeof this.model.properties || !1 !== this.model.properties["saiku.olap.query.automatic_execution"] || !1 !== a && void 0 !== a && null !== a) {
            ...
            var d = !1, e = '\x3cspan class\x3d"i18n"\x3eQuery Validation failed!\x3c/span\x3e', f = this.helper.model(), g;
            ...
            "OLAP" == f.queryType && ("QUERYMODEL" == f.type ? (g = 0 < Object.keys(f.queryModel.axes.COLUMNS.hierarchies).length,
            h = 0 < Object.keys(f.queryModel.axes.ROWS.hierarchies).length,
            k = "COLUMNS" == f.queryModel.details.axis && 0 < f.queryModel.details.measures.length,
            h && g && k || (e = ""),
            g || k || (e += '\x3cspan class\x3d"i18n"\x3eYou need to include at least one measure or a level on columns for a valid query.\x3c/span\x3e'),
            h || (e += '\x3cspan class\x3d"i18n"\x3eYou need to include at least one level on rows for a valid query.\x3c/span\x3e'),
            (g || k) && h && (d = !0)) : "MDX" == f.type && ((d = f.mdx && 0 < f.mdx.length) || (e = '\x3cspan class\x3d"i18n"\x3eYou need to enter some MDX statement to execute.\x3c/span\x3e')));
            d ? (this.workspace.table.clearOut(),
            $(this.workspace.processing).html('\x3cspan class\x3d"processing_image"\x3e\x26nbsp;\x26nbsp;\x3c/span\x3e \x3cspan class\x3d"i18n"\x3eRunning query...\x3c/span\x3e [\x26nbsp;\x3ca class\x3d"cancel i18n" href\x3d"#cancel"\x3eCancel\x3c/a\x3e\x26nbsp;]').show(),
            this.workspace.adjust(),
            this.workspace.trigger("query:fetch"),
            Saiku.i18n.translate(),
            this.workspace.block('\x3cspan class\x3d"processing_image"\x3e\x26nbsp;\x26nbsp;\x3c/span\x3e \x3cspan class\x3d"i18n"\x3eRunning query...\x3c/span\x3e [\x26nbsp;\x3ca class\x3d"cancel i18n" href\x3d"#cancel"\x3eCancel\x3c/a\x3e\x26nbsp;]'),
            this.result.save({}, {
            ...
            $(this.workspace.processing).html(e).show(),
            this.workspace.adjust(),
            Saiku.i18n.translate())
        }
    },
  ...
  , Session = Backbone.Model.extend({
    ...
    initialize: function(a, b) {
        ...
        b && b.username && b.password ? (this.username = b.username,
        this.password = b.password,
        Settings.DEMO ? this.check_session() : this.save({
    ...
    check_session: function() {
        null === this.sessionid || null === this.username || null === this.password ? (this.clear(),
        this.fetch({
    ...
    process_session: function(a, b) {
        null === b || null == b.sessionid ? (Saiku.ui.unblock(),
        this.form = Settings.DEMO ? new DemoLoginForm({
            session: this
        }) : new LoginForm({
            session: this
        }),
        ...
        this.username = encodeURIComponent(b.username),
        this.language = b.language,
        "undefined" != typeof this.language && this.language != Saiku.i18n.locale && (Saiku.i18n.locale = this.language,
        Saiku.i18n.automatic_i18n()),
        (new License).fetch_license("api/license/", function(a) {
  ...
  , SplashScreen = Backbone.View.extend({
    ...
    click_head: function(a) {
        ...
        $(".stabs section").hide();
        b = "";
        -1 < a.indexOf("welcome") ? b = "welcome" : -1 < a.indexOf("features") ? b = "features" : -1 < a.indexOf("help") ? b = "help" : -1 < a.indexOf("enterprise") && (b = "enterprise");
        $("#" + b).fadeIn()
    },
    run_tour: function() {
    ...
    render: function() {
        new License;
        Settings.BIPLUGIN5 ? ($(this.el).html(this.template()),
        void 0 != Settings.LICENSE.licenseType && "trial" != Settings.LICENSE.licenseType && "Open Source License" != Settings.LICENSE.licenseType && $(this.el).find(".enterprisetoggle").css("visibility", "hidden"),
        this.getContent(),
        this.getNews(),
        this.setupPage(this),
        $("#splash").find("\x3e nav \x3e ul \x3e li.active \x3e a").click(function() {
        ...
        this.getNews(),
        this.setupPage(this),
        $("#splash \x3e nav \x3e ul \x3e li.active \x3e a").click(function() {
            var a = $(this).attr("class");
            $("nav li").removeClass("active");
            $(this).parent().addClass("active");
            $(".stabs section").hide();
    ...
    getNews: function() {
        ...
        $.ajax({
            ...
            success: function(b) {
                for (var c = 0; c < b.item.length; c++)
                    $(a.el).find("#news").append("\x3ch4 style\x3d'margin-left: 0.5%;color:#6D6E71;'\x3e" + b.item[c].title + "\x3c/h4\x3e\x3cstrong style\x3d'margin-left: 0.5%;color:#6D6E71;'\x3e" + b.item[c].date + "\x3c/strong\x3e\x3cbr/\x3e\x3cp style\x3d'color:#6D6E71;'\x3e" + b.item[c].body + "\x3c/p\x3e")
            },
            error: function(a) {
  ...
  , SessionWorkspace = Backbone.Model.extend({
    initialize: function(a, b) {
        ...
        this.initialized = !1;
        this.first = !0;
        "undefined" !== typeof localStorage && localStorage && (Settings.LOCALSTORAGE_EXPIRATION && 0 !== Settings.LOCALSTORAGE_EXPIRATION || localStorage.clear(),
        localStorage.getItem("expiration") && localStorage.getItem("expiration") <= (new Date).getTime() ? localStorage.clear() : localStorage.getItem("saiku-version") && localStorage.getItem("saiku-version") === Settings.VERSION || (localStorage.clear(),
        ...
        for (var c = this, d = 0; d < a.length; d++)
            if (a[0].visible) {
                ...
                if (filePath.indexOf(".saiku") === filePath.length - 6) {
                    ...
                    filePath.indexOf(".sdb") === filePath.length - 4 && (Saiku.Dashboards ? (Saiku.ui.block("Loading default report..."),
                    Saiku.tabs.add(new DashboardViewTab({
                        file: a[d].path
                    }))) : !Settings.INITIAL_QUERY && paramsURI.splash ? Saiku.tabs.add(new Workspace) : Settings.INITIAL_QUERY || Saiku.tabs.add(new Workspace));
                break
            } else
                a.shift()
    },
    process_datasources: function(a, b) {
        ...
        if (this.initialized)
            ...
            if (!_.has(d, "splash"))
                d.splash = !0;
            else if (_.has(d, "splash") && d.splash || _.has(d, "splash") && null === d.splash)
                d.splash = !0;
            if (Settings.DEFAULT_REPORT_SHOW) {
                var e = function() {
                    var a, b;
                    if (0 < _.size(Settings.DEFAULT_REPORTS) && 0 < k.length)
                            ...
                            if (k[0].visible) {
                                ...
                                if (a.indexOf(".saiku") === a.length - 6) {
                                    ...
                                    e.bind("report:rendered", function() {
                                        h.render_default_tab(k, e)
                                    })
                                } else
                                    a.indexOf(".sdb") === a.length - 4 && (Saiku.Dashboards ? (Saiku.ui.block("Loading default report..."),
                                    Saiku.tabs.add(new DashboardViewTab({
                                        file: k[b].path
                                    })),
                                    k.shift(),
                                    h.render_default_tab(k, null )) : !Settings.INITIAL_QUERY && d.splash ? Saiku.tabs.add(new Workspace) : Settings.INITIAL_QUERY || Saiku.tabs.add(new Workspace));
                                break
                            } else
                                k.shift();
                    else
                        !Settings.INITIAL_QUERY && d.splash ? Saiku.tabs.add(new Workspace) : Settings.INITIAL_QUERY || Saiku.tabs.add(new Workspace)
                }
                ;
                !Settings.INITIAL_QUERY && d.splash ? Saiku.tabs.add(new Workspace) : Settings.INITIAL_QUERY || Saiku.tabs.add(new Workspace);
                ...
                var g = 0
                  ...
                  , g = setInterval(function() {
                    "undefined" !== typeof window.DashboardViewTab && (clearInterval(g),
                    e())
                }, 100)
            } else
                !Settings.INITIAL_QUERY && d.splash ? Saiku.tabs.add(new Workspace) : Settings.INITIAL_QUERY || Saiku.tabs.add(new Workspace)
    ...
    prefetch_dimensions: function() {
        ...
        !this.initialized && Backbone.history && (Backbone.history.start(),
        this.initialized = !0)
    },
    url: function() {
        return this.first ? (this.first = !1,
        encodeURI(Saiku.session.username + "/discover")) : encodeURI(Saiku.session.username + "/discover/refresh")
    }
})
  ...
  , Saiku = {
    ...
    log: function(a, b) {
        console && console.log && (console.log("Logging for: " + a),
        b && console.log(b))
    },
    error: function(a, b) {
        console && console.error && (console.error("Logging for: " + a),
        console.error(b))
    },
    URLParams: {
        ...
        paramsURI: function() {
            var a = {}, b = window.location.search.substr(1).split("\x26"), c = b.length, d, e;
            if (1 < window.location.search.length)
                for (d = 0; d < c; d++)
                    e = b[d].split("\x3d"),
                    a[decodeURIComponent(e[0])] = 1 < e.length ? this.buildValue(decodeURIComponent(e[1])) : null ;
            return a
        },
        equals: function() {
    ...
    toPattern: function(a, b) {
        var c = ("object" === typeof b ? b.pattern : b).split(""), d = a.toString().replace(/[^0-9a-zA-Z]/g, ""), e = 0, f = c.length, g;
        for (g = 0; g < f && !(e >= d.length); g++)
            if ("9" === c[g] && d[e].match(/[0-9]/) || "A" === c[g] && d[e].match(/[a-zA-Z]/) || "S" === c[g] && d[e].match(/[0-9a-zA-Z]/))
                c[g] = d[e++];
            else if ("9" === c[g] || "A" === c[g] || "S" === c[g])
                c = c.slice(0, g);
        ...
        success: function() {
            ...
            b.fetch({
                success: function() {
                    ...
                    a.each(function(a) {
                        d += 1;
                        if ("js/saiku/plugins/I18n/plugin.js" != a.attributes.path)
                            jQuery.ajax({
                                async: !1,
                                type: "GET",
                ...
                error: function() {
                    ...
                    a.each(function(a) {
                        ...
                        "js/saiku/plugins/I18n/plugin.js" != a.attributes.path ? jQuery.ajax({
                            ...
                            data: null ,
                            success: function() {
                                d == b && (void 0 != Settings.CSS && Saiku.loadCSS(Settings.CSS, null ),
...
var DateFilterModal = Modal.extend({
    ...
    initialize: function(a) {
        ...
        this.bind("open", this.post_render);
        this.render();
        this.show_button_clear() ? (this.$el.find(".dialog_footer a:nth-child(1)").show(),
        this.$el.find(".dialog_footer a:nth-child(3)").hide()) : this.$el.find(".dialog_footer a:nth-child(1)").hide();
        this.$el.parent().find(".ui-dialog-titlebar-close").bind("click", this.finished);
        this.member = new Member({},{
            cube: this.workspace.selected_cube,
            dimension: this.key
    ...
    save_data_levels: function() {
        ...
        _.each(this.data.hierarchies.levels, function(c, d, e) {
            if (void 0 !== e[d].annotations.AnalyzerDateFormat || void 0 !== e[d].annotations.SaikuDayFormatString)
                void 0 !== e[d].annotations.AnalyzerDateFormat ? b.push({
                    name: e[d].name,
    ...
    add_values_fixed_date: function() {
        ...
        this.$el.find(".available-selections").each(function(b, c) {
            "fixed-date" === $(c).attr("selection-name") && ($(c).find("input:radio").each(function(b, c) {
            ...
            $(c).find("input:radio").each(function(a, b) {
                null !== $(b).val() && void 0 !== $(b).val() && "" !== $(b).val() && "on" !== $(b).val() || $(b).addClass("keep-disabled")
            }))
        })
    },
    add_values_last_periods: function() {
        ...
        this.$el.find(".available-selections").each(function(b, c) {
            ...
            $(c).find("#period-select \x3e option").each(function(a, b) {
                null !== $(b).attr("value") && void 0 !== $(b).attr("value") && "" !== $(b).attr("value") || $(b).addClass("keep-disabled")
            }))
        })
    },
    ...
    add_selected_date: function(a) {
        ...
        "" !== a.val() ? (c = Saiku.toPattern(a.val(), c),
        a.css("border", "1px solid #ccc"),
        b.append($("\x3cli\x3e\x3c/li\x3e").text(c).append('\x3ca href\x3d"#" class\x3d"del-date" data-date\x3d"' + c + '"\x3ex\x3c/a\x3e')),
        this.selectedDates.push(c)) : a.css("border", "1px solid red");
        a.val("")
    },
    ...
    populate: function() {
        var a = this.get_date_filter(), b;
        if (a && !_.isEmpty(a))
            if ("operator" === a.type) {
                var c = this.$el.find("#" + a.checked)
                  , d = c.data("operator")
                  , e = this;
                ...
                "after" === d || "after\x26equals" === d || "before" === d || "before\x26equals" === d ? this.$el.find("#selection-date").val(this.selectedDates[0]) : "between" === d ? (e.$el.find("#start-date").val(this.selectedDates[0]),
                e.$el.find("#end-date").val(this.selectedDates[1])) : "notbetween" === d ? (e.$el.find("#start-date").val(this.selectedDates[0]),
                ...
                "fixed-date" === a.type ? (b = this.$el.find("#selection-radio-fixed-date"),
                b.prop("checked", !0),
                this.$el.find("#" + a.checked).prop("checked", !0)) : (b = this.$el.find("#selection-radio-available"),
                b.prop("checked", !0),
                this.$el.find("#date-input").val(a.periodAmount),
                this.$el.find('select#period-select option[id\x3d"' + a.periodSelect + '"]').prop("selected", !0)),
                this.disable_divselections(b, !0)
    },
    populate_mdx: function(a, b, c) {
        ...
        a.workinglevel !== a.level && void 0 !== a.workinglevel ? (a.parent = "[{dimension}.{hierarchy}].[{level}].members,",
        ...
        if ("dayperiods" === b) {
            ...
            if (1 < this.selectedDates.length) {
                ...
                for (c = 0; c < b; c++)
                    if (a.dates = this.selectedDates[c],
                    "\x3e\x3d|\x3c\x3d" === a.comparisonOperator)
                        0 === c ? (a.comparisonOperator = a.comparisonOperator.split("|")[0],
                        this.template_days_mdx = this.template_days_mdx.replace(/{(\w+)}/g, function(b, c) {
                            return a[c]
                        }),
                        a.comparisonOperator = "\x3e\x3d|\x3c\x3d") : (a.logicalOperator = "AND",
                        a.comparisonOperator = a.comparisonOperator.split("|")[1],
                        this.template_days_mdx += this.template_many_years_mdx.replace(/{(\w+)}/g, function(b, c) {
                            return a[c]
                        }));
                    else if ("\x3e\x3d||\x3c\x3d" === a.comparisonOperator)
                        if (0 === c)
                            this.template_days_mdx = "EXCEPT({parent}.Members, " + this.template_days_mdx,
                            a.comparisonOperator = a.comparisonOperator.split("||")[0],
                            this.template_days_mdx = this.template_days_mdx.replace(/{(\w+)}/g, function(b, c) {
                                return a[c]
                            }),
                            a.comparisonOperator = "\x3e\x3d||\x3c\x3d";
                        else
                            return a.logicalOperator = "AND",
                            a.comparisonOperator = a.comparisonOperator.split("||")[1],
                            this.template_days_mdx += this.template_many_years_mdx.replace(/{(\w+)}/g, function(b, c) {
                                return a[c]
                            }),
                        ...
                        0 === c ? this.template_days_mdx = this.template_days_mdx.replace(/{(\w+)}/g, function(b, c) {
                            return a[c]
                        }) : (a.logicalOperator = "\x3c\x3e" === a.comparisonOperator ? "AND" : "OR",
                        this.template_days_mdx += this.template_many_years_mdx.replace(/{(\w+)}/g, function(b, c) {
                            return a[c]
                        }));
    ...
    save: function(a) {
        ...
        $(this.el).find(".dialog_body").children().hide();
        $(this.el).find(".dialog_body").prepend(a);
        var b = this, c, d, e, f, g = null , h = {};
        if (null === b.hierarchy || void 0 === b.hierarchy)
            b.hierarchy = b.dimension;
            ...
            if ("true" === $(k).attr("available")) {
                ...
                "operator" === $(k).attr("selection-name") ? $(k).find("input:radio").each(function(a, e) {
                    if (!0 === $(e).is(":checked")) {
                        ...
                        h.checked = $(e).attr("id");
                        if ("after" === f || "after\x26equals" === f || "before" === f || "before\x26equals" === f)
                            b.selectedDates = [],
                            b.selectedDates.push(b.$el.find("#selection-date").val());
                        else if ("between" === f || "notbetween" === f)
                            b.selectedDates = [],
                            b.selectedDates.push(b.$el.find("#start-date").val()),
                            b.selectedDates.push(b.$el.find("#end-date").val());
                        c = "dayperiods";
                        d = $(e).val();
                        h.values = b.selectedDates
                    }
                }) : "fixed-date" === $(k).attr("selection-name") ? $(k).find("input:radio").each(function(a, b) {
                    !0 === $(b).is(":checked") && (c = $(b).attr("id").split("-")[1],
                    e = $(b).val(),
                    h.checked = $(b).attr("id"))
                }) : "rolling-date" === $(k).attr("selection-name") && (e = $("#period-select").find(":selected").val(),
                c = "lastperiods",
                f = $(k).find("input:text").val(),
                h.fixedDateName = c,
                h.periodAmount = $(k).find("input:text").val(),
                h.periodSelect = $("#period-select").find(":selected").attr("id"));
                var l = b.dataLevels.length, q, s;
                for (s = 0; s < l; s++)
                    b.dataLevels[s].analyzerDateFormat === e && (q = b.dataLevels[s].name);
                l = {
                ...
                g = "dayperiods" === c && "" !== b.selectedDates[0] && void 0 !== b.selectedDates[0] || "lastperiods" === c && !_.isEmpty(e) && "Day(s)" !== e && !_.isEmpty(f) || "dayperiods" !== c && "lastperiods" !== c && !_.isEmpty(e) ? b.populate_mdx(l, c) : null
            }
        });
        ...
        if ("dayperiods" === c && "" !== this.selectedDates[0] && void 0 !== b.selectedDates[0] || "lastperiods" === c && !_.isEmpty(e) && "Day(s)" !== e && !_.isEmpty(f) || "dayperiods" !== c && "lastperiods" !== c && !_.isEmpty(e))
            this.set_date_filter(h);
        else {
    ...
    set_date_filter: function(a) {
        var b = this.workspace.dateFilter
          , c = b.toJSON()
          , d = this.get_uuid(a);
        a.id = d;
        a.key = this.key;
        c && !_.isEmpty(c) ? b.get(d) ? (b = b.get(d),
        b.set(a)) : b.add(a) : b.add(a);
    ...
    show_button_clear: function() {
        var a = this.workspace.dateFilter, b = a.toJSON(), c;
  ...
  , DateFilterObserver = Backbone.View.extend({
    ...
    receive_data: function(a) {
        ...
        if (b && !_.isEmpty(b))
            return _.delay(this.workspace_levels, 1E3, a)
    },
    get_cube_name: function() {
        return decodeURIComponent(this.workspace.selected_cube.split("/")[3])
    },
    workspace_levels: function(a) {
        ...
        var b = this.workspace.query.helper.getAxis("COLUMNS")
          ...
          , e = [];
        "COLUMNS" === b.location && 0 < b.hierarchies.length && e.push(this.get_axes(a, b));
    ...
    get_axes: function(a, b) {
        var c = [], d = b.hierarchies.length, e;
        for (e = 0; e < d; e++)
            for (var f in b.hierarchies[e].levels)
                b.hierarchies[e].levels.hasOwnProperty(f) && c.push("[" + a + "]." + b.hierarchies[e].name + ".[" + f + "]");
        return c
    },
    check_dateFilter_saved: function() {
        var a = this.workspace.checkDateFilterSaved;
        this.workspace.item && !_.isEmpty(this.workspace.item) && void 0 === a && (a = this.workspace.query.getProperty("saiku.ui.datefilter.data"),
        this.workspace.dateFilter.add(a));
    ...
    check_dateFilter_model: function(a) {
        ...
        if (0 < f && d && !_.isEmpty(d))
            for (; h < f; )
                a[h] === d[g].id ? c.push(d[g].id) : b.push(d[g].id),
                g + 1 < e ? g++ : (g = 0,
                h++);
        else if (0 === f && d && !_.isEmpty(d)) {
            for (a = 0; a < e; a++)
                this.workspace.dateFilter.remove(d[a].id);
            this.workspace.query.setProperty("saiku.ui.datefilter.data", this.workspace.dateFilter.toJSON())
        }
    ...
    remove_dateFilter_model: function(a) {
        var b = a.length, c;
        for (c = 0; c < b; c++)
            this.workspace.dateFilter.remove(a[c]);
        this.workspace.query.setProperty("saiku.ui.datefilter.data", this.workspace.dateFilter.toJSON())
    }
});
Saiku.events.bind("session:new", function() {
    ...
    for (var b = 0, c = Saiku.tabs._tabs.length; b < c; b++)
        a({
...
(function(a) {
    ...
    a.Base64 || (a.Base64 = {
        encode: function(a) {
            for (var c, f, g, h, k, l, m = 0, n = a.length, r = Math.max, q = ""; m < n; ) {
                c = a.charCodeAt(m++) || 0;
                f = a.charCodeAt(m++) || 0;
                l = a.charCodeAt(m++) || 0;
                if (255 < r(c, f, l))
                ...
                k = l & 63;
                f ? l || (k = 64) : h = k = 64;
                q += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/\x3d".charAt(g) + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/\x3d".charAt(c) + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/\x3d".charAt(h) + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/\x3d".charAt(k)
            }
            return q
        }
    })
...
Backbone.sync = function(a, b, c) {
    ...
    b = {
        ...
        error: function(a, b, f) {
            !isIE && "undefined" != typeof console && console && console.error && (console.error("Error performing " + d + " on " + e),
            console.error(f));
            c.error && c.error(a, b, f)
        },
        async: a
    };
    ...
    if (Settings.BIPLUGIN && !Settings.BIPLUGIN5 || Backbone.emulateHTTP)
        if ("PUT" === d || "DELETE" === d)
            Backbone.emulateHTTP && (b.data._method = d),
            b.type = "POST",
